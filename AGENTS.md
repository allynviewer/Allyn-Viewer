...
- Commands (repo root):
 ...
 - Packages: `build.bat package` → `build-vc-64\dist\Allyn_Viewer_<Type>_<ver>_x86_64.zip` (portable, always) + `..._Setup.exe` (NSIS, only when NSIS + StdUtils/INetC plugins are installed; `scripts\setup-nsis-plugins.ps1` installs the plugins). Both come from `viewer_manifest.py` (`package_zip()` / `package_finish()`), same file list.
