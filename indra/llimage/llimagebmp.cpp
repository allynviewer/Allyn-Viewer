...
BOOL LLImageBMP::decodeColorMask16( U8* dst, U8* src )
{
	...
	for( S32 row = 0; row < getHeight(); row++ )
	{
		...
		{
			U32 value = *((U16*)src);
			dst[0] = U8((value & mBitfieldMask[2]) >> r_shift);
			dst[1] = U8((value & mBitfieldMask[1]) >> g_shift);
			dst[2] = U8((value & mBitfieldMask[0]) >> b_shift);
			src += 2;
			dst += 3;
		}
		src += alignment_bytes;
...
BOOL LLImageBMP::decodeColorMask32( U8* dst, U8* src )
{
	...
	for( S32 row = 0; row < getHeight(); row++ )
	{
		...
		{
			U32 value = *((U32*)src);
			dst[0] = U8((value & mBitfieldMask[0]) >> r_shift);
			dst[1] = U8((value & mBitfieldMask[1]) >> g_shift);
			dst[2] = U8((value & mBitfieldMask[2]) >> b_shift);
			src += 4;
			dst += 3;
		}
		src += alignment_bytes;
...
BOOL LLImageBMP::decodeColorTable8( U8* dst, U8* src )
{
	...
	for( S32 row = 0; row < getHeight(); row++ )
	{
		...
		{
			S32 index = 4 * src[0];
			dst[0] = mColorPalette[index + 2];
			dst[1] = mColorPalette[index + 1];
			dst[2] = mColorPalette[index + 0];
			src++;
			dst += 3;
		}
		src += alignment_bytes;
...
BOOL LLImageBMP::encode(const LLImageRaw* raw_image, F32 encode_time)
{
	...
	header.mImageSize = 0;
#if LL_DARWIN
	header.mHorzPelsPerMeter = header.mVertPelsPerMeter = 2834;
#else
