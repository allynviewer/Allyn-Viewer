...
void LLDataPackerBinaryBuffer::dumpBufferToLog()
{
	...
	{
		snprintf(line_buffer + cur_line_pos*3, sizeof(line_buffer) - cur_line_pos*3, "%02x ", mBufferp[i]);
		cur_line_pos++;
		if (cur_line_pos >= 16)
		{
			cur_line_pos = 0;
...
BOOL LLDataPackerAsciiBuffer::unpackString(std::string& value, const char *name)
{
	...
	BOOL res = getValueStr(name, valuestr, DP_BUFSIZE);
	if (!res)
	{
		return FALSE;
	}
...
BOOL LLDataPackerAsciiBuffer::packBinaryData(const U8 *value, S32 size, const char *name)
{
	...
	if (mWriteEnabled)
	{
		numCopied = snprintf(mCurBufferp,getBufferSize()-getCurrentSize(),"%010d ", size);
		if (numCopied < 0 || numCopied > getBufferSize()-getCurrentSize())
		{
			numCopied = getBufferSize()-getCurrentSize();
		...
		{
			numCopied = snprintf(mCurBufferp, getBufferSize()-getCurrentSize(), "%02x ", value[i]);
			if (numCopied < 0 || numCopied > getBufferSize()-getCurrentSize())
			{
				numCopied = getBufferSize()-getCurrentSize();
...
void LLDataPackerAsciiBuffer::writeIndentedName(const char *name)
{
	if (mIncludeNames)
	{
		...
		if (mWriteEnabled)
		{
			numCopied = snprintf(mCurBufferp,getBufferSize()-getCurrentSize(),"%s\t", name);
		}
		else
		{
