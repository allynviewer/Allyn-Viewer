...
namespace
{
	...
	public:
		...
		AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return fnPtrResponder_timeout; }
		char const* getName(void) const { return "LLFnPtrResponder"; }
	private:
							...
							const LLSD& context, const LLSD& input) const
{
	...
	LLMessageReader* previous_reader = gMessageSystem->mMessageReader;
	gMessageSystem->mMessageReader = gMessageSystem->mLLSDMessageReader;
	const bool handled = gMessageSystem->callHandler(namePtr, false, gMessageSystem);
	gMessageSystem->mMessageReader = previous_reader;
	if(handled)
	{
...
void LLMessageSystem::init()
{
    ...
    mDroppedPackets = 0;
    mResentPackets = 0;
    mFailedResendPackets = 0;
    mOffCircuitPackets = 0;
    mInvalidOnCircuitPackets = 0;
	mOurCircuitCode = 0;
											...
											void** user)
{
	...
	if (circuit_code_in)
	{
		...
		{
			cdp->checkPacketInID(gMessageSystem->mCurrentRecvPacketID, FALSE );
		}
...
namespace
{
	...
	std::string serialize_eq_llsd(const LLSD& src)
	{
	...
	LLSD deserialize_eq_llsd(const std::string& blob)
	{
		LLSD dst;
	...
	void dispatchPendingEventQueue()
	{
		...
		{
			LLMessageSystem::dispatch(item.name, deserialize_eq_llsd(item.message_bin));
		}
	}
}
	...
	const LLSD& message)
{
	if (!AIThreadID::in_main_thread())
	{
		...
		item.name = msg_name;
		item.message_bin = serialize_eq_llsd(message);
		{
			std::lock_guard<std::mutex> lock(sPendingEQMutex);
...
void end_messaging_system(bool print_summary)
{
	...
	LLTransferTargetVFile::updateQueue(true);
	if (gMessageSystem)
	{
	...
	const LLUUID& id2) const
{
	...
	d.update((const unsigned char *) colon, (U32)strlen(colon));
	snprintf(tbuf, sizeof(tbuf),"%i", number);
	d.update((unsigned char *) tbuf, (U32)strlen(tbuf));
	d.update((const unsigned char *) colon, (U32)strlen(colon));
	if( (char*) id1str != NULL)
	{
...
bool LLMessageSystem::isMatchingDigestForWindowAndUUIDs(const char* digest, const S32 window, const LLUUID &id1, const LLUUID &id2) const
{
	...
	char our_digest[MD5HEX_STR_SIZE];
	U32 now = (U32)time(NULL);
	now /= window;
