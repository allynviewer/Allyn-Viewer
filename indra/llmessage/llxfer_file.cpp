...
{
 	S32 retval = 0;
	mFp = LLFile::fopen(mTempFilename,"w+b");
	if (mFp)
	{
		fclose(mFp);
		mFp = NULL;
...
S32 LLXfer_File::startSend (U64 xfer_id, const LLHost &remote_host)
{
	...
	mFp = LLFile::fopen(mLocalFilename,"rb");
	if (mFp)
	{
...
S32 LLXfer_File::flush()
{
	...
	if (mBufferLength)
	{
		...
		mFp = LLFile::fopen(mTempFilename,"a+b");
		if (mFp)
		{
...
S32 copy_file(const std::string& from, const std::string& to)
{
	...
	if(in && out)
	{
		...
		while(((read = fread(buffer, 1, sizeof(buffer), in)) > 0)
			  && (fwrite(buffer, 1, read, out) == (U32)read));
