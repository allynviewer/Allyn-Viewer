...
const S32 LLExperienceCache::DEFAULT_QUOTA			= 128;
...
void LLExperienceCache::idleCoro()
{
    ...
    const F32 ERASE_EXPIRED_TIMEOUT = 60.f;
    {
...
F64 LLExperienceCacheImpl::getErrorRetryDeltaTime(S32 status, const AIHTTPReceivedHeaders& headers)
{
    ...
    if (status == 503)
    {
        const F64 SERVICE_UNAVAILABLE_DELAY = 600.0;
        return SERVICE_UNAVAILABLE_DELAY;
    }
    else if (status == 499)
    {
        const F64 BUSY_DELAY = 10.0;
        return BUSY_DELAY;
    }
    else
    {
        const F64 DEFAULT_DELAY = 3600.0;
