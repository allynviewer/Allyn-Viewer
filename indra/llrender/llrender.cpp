...
LLMatrix4a LLRender::genRot(const GLfloat& a, const LLVector4a& axis) const
{
	...
	F32 s = sinf(r);
	F32 ic = 1.f-c;
	const LLVector4a add1(c,axis[VZ]*s,-axis[VY]*s);
	const LLVector4a add2(-axis[VZ]*s,c,axis[VX]*s);
	const LLVector4a add3(axis[VY]*s,-axis[VX]*s,c);
	LLVector4a axis_x;
	axis_x.splat<0>(axis);
	LLVector4a axis_y;
	axis_y.splat<1>(axis);
...
bool LLRender::unprojectf(const LLVector3& windowCoordinate, const LLMatrix4a& modelview, const LLMatrix4a& projection, const LLRect& viewport, LLVector3& object)
{
	...
	norm_view.mul(two);
	norm_view.sub(one);
	inv_mat.rotate4(norm_view,temp_vec);
	w.splat<3>(temp_vec);
	LLVector4a div;
	div.setSelectWithMask( w.equal( _mm_setzero_ps() ), one, w );
	temp_vec.div(div);
