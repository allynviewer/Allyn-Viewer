...
#if LL_WINDOWS
std::set<std::string> sWGLExtensions;
#endif
...
void loadWGLExtensionStrings()
{
	...
	if (wglGetExtensionsStringARB)
	{
		typedef boost::tokenizer<boost::char_separator<char> > tokenizer;
		boost::char_separator<char> sep(" ");
		const char* ext_str = wglGetExtensionsStringARB(wglGetCurrentDC());
		if (!ext_str) return;
		std::string extensions = std::string(ext_str);
		for (auto& extension : tokenizer(extensions, sep))
		{
...
# endif
	...
	if (getenv("LL_GL_BLACKLIST"))
	{
		...
		if (strchr(blacklist,'d')) mHasMipMapGeneration = FALSE;
		if (strchr(blacklist,'i')) mHasAnisotropic = FALSE;
		if (strchr(blacklist,'j')) mHasCubeMap = FALSE;
		if (strchr(blacklist,'l')) mHasOcclusionQuery = FALSE;
		if (strchr(blacklist,'m')) mHasShaderObjects = FALSE;
		if (strchr(blacklist,'n')) mHasVertexShader = FALSE;
		if (strchr(blacklist,'o')) mHasFragmentShader = FALSE;
		if (strchr(blacklist,'p')) mHasPointParameters = FALSE;
		if (strchr(blacklist,'q')) mHasFramebufferObject = FALSE;
		if (strchr(blacklist,'r')) mHasDrawBuffers = FALSE;
		if (strchr(blacklist,'s')) mHasFramebufferMultisample = FALSE;
		if (strchr(blacklist,'u')) mHasBlendFuncSeparate = FALSE;
