...
public:
	...
	bool isEnabled()					const	{return mShader.mProgramObject && mEnabled;}
	void bindShader()							{mShader.bind();}
	void unbindShader()							{mShader.unbind();}
	LLGLSLShader& getShader()					{return mShader;}
	LLSD getDefaults();
	void loadSettings(const LLSD& settings);
	void addSetting(IShaderSettingBase& setting) { mSettings.push_back(&setting); }
protected:
	template<typename T>
	struct LLShaderSetting : public IShaderSettingBase
	{
		...
		const std::string& getName()	const	{ return mSettingName; }
		LLSD getDefaultValue()			const	{ return mDefault; }
...
private:
	std::vector<IShaderSettingBase*> mSettings;
...
public:
	...
	S32 getColorChannel()	const	{return 0;}
	S32 getDepthChannel()	const	{return -1;}
...
public:
	...
	bool isEnabled()		const	{ return LLPostProcessShader::isEnabled() && mNumPasses.get(); }
	S32 getColorChannel()	const	{ return 0; }
	S32 getDepthChannel()	const	{ return -1; }
	QuadType preDraw()
	{
...
public:
	...
	bool isEnabled()		const	{ return LLPostProcessShader::isEnabled() && llabs(gGLModelView.getF32ptr()[0] - gGLPreviousModelView.getF32ptr()[0]) > .0000001; }
	S32 getColorChannel()	const	{ return 0; }
	S32 getDepthChannel()	const	{ return 1; }
	QuadType preDraw()
	{
