...
public:
	...
	BOOL				loadLayersets();
public:
	LLJoint*		getCharacterJoint(U32 num);
	const char*		getAnimationPrefix() { return "avatar"; }
	LLVector3		getVolumePos(S32 joint_index, LLVector3& volume_offset);
	LLJoint*		findCollisionVolume(S32 volume_id);
	S32				getCollisionVolumeID(std::string &name);
	LLPolyMesh*		getHeadMesh();
	LLPolyMesh*		getUpperBodyMesh();
...
public:
	...
	LLJoint*	getRootJoint() { return mRoot; }
