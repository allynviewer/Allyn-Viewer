...
U32 LLPolyMeshSharedData::getNumKB()
{
	...
	{
		num_kb += mNumVertices * sizeof(LLVector2);
	}
	if (mHasWeights && !isLOD())
	{
		num_kb += mNumVertices * sizeof(float);
	}
	num_kb += mNumFaces * sizeof(LLPolyFace);
	num_kb /= 1024;
	return num_kb;
}
...
BOOL LLPolyMeshSharedData::loadMesh( const std::string& fileName )
{
	...
	if ( strncmp(header, HEADER_BINARY, strlen(HEADER_BINARY)) == 0 )
	{
		...
		if (!isLOD())
		{
			...
			char morphName[64+1];
			morphName[sizeof(morphName)-1] = '\0';
...
BOOL LLPolyMesh::saveLLM(LLFILE *fp)
{
	...
	if (!isLOD())
	{
		...
		numVertices = mSharedData->mNumVertices;
		for (U16 i = 0; i < numVertices; ++i)
		{
	...
	numFaces = mSharedData->mNumFaces;
