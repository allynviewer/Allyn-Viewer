...
public:
	...
	LLViewerVisualParam* cloneParam(LLWearable* wearable) const = 0;
protected:
...
public:
	...
	LLViewerVisualParam* cloneParam(LLWearable* wearable = NULL) const;
	void		apply( ESex avatar_sex ) {}
	void		setWeight(F32 weight, bool upload_bake = false);
	void		setAnimationTarget(F32 target_value, bool upload_bake = false);
	void		animate(F32 delta, bool upload_bake = false);
	char const* getTypeString(void) const { return "param_alpha"; }
	F32					getTotalDistortion()									{ return 1.f; }
	const LLVector4a&	getAvgDistortion()										{ return mAvgDistortionVec; }
	F32					getMaxDistortion()										{ return 3.f; }
	LLVector4a			getVertexDistortion(S32 index, LLPolyMesh *poly_mesh)	{ return LLVector4a(1.f, 1.f, 1.f);}
	const LLVector4a*	getFirstDistortion(U32 *index, LLPolyMesh **poly_mesh)	{ index = 0; poly_mesh = NULL; return &mAvgDistortionVec;};
	const LLVector4a*	getNextDistortion(U32 *index, LLPolyMesh **poly_mesh)	{ index = 0; poly_mesh = NULL; return NULL;};
...
public:
	...
	void			setAnimationTarget(F32 target_value, bool upload_bake = false);
	void			animate(F32 delta, bool upload_bake = false);
	char const* getTypeString(void) const { return "param_color"; }
	F32					getTotalDistortion()									{ return 1.f; }
	const LLVector4a&	getAvgDistortion()										{ return mAvgDistortionVec; }
	F32					getMaxDistortion()										{ return 3.f; }
	LLVector4a			getVertexDistortion(S32 index, LLPolyMesh *poly_mesh)	{ return LLVector4a(1.f, 1.f, 1.f); }
