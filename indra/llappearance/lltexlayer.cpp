...
BOOL LLTexLayerInterface::setInfo(const LLTexLayerInfo *info, LLWearable* wearable  )
{
	if (mInfo != NULL)
	{
...
void LLTexLayer::gatherAlphaMasks(U8 *data, S32 originX, S32 originY, S32 width, S32 height)
{
...
void LLTexLayerTemplate::gatherAlphaMasks(U8 *data, S32 originX, S32 originY, S32 width, S32 height)
{
	U32 num_wearables = updateWearableCache();
	U32 i = num_wearables - 1;
	LLTexLayer *layer = getLayer(i);
	if (layer)
	{
