...
public:
	...
	BOOL		setInfo(const LLTexLayerInfo *info, LLWearable* wearable);
	BOOL		blendAlphaTexture(S32 x, S32 y, S32 width, S32 height);
	void		gatherAlphaMasks(U8 *data, S32 originX, S32 originY, S32 width, S32 height);
	void		setHasMorph(BOOL newval);
	void		deleteCaches();
	BOOL		isInvisibleAlphaMask() const;
protected:
