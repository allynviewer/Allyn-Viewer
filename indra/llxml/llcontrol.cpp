	...
	if (existing_control)
 	{
		if (persist && existing_control->isType(type))
		{
			...
			{
				LLSD cur_value = existing_control->getValue();
				existing_control->setDefaultValue(initial_val);
				existing_control->setValue(cur_value);
...
{
	gCOAEnabled = !!newvalue.asInteger();
...
#define TEST_LLCC(T, V) if((T)mySetting_##T != V) LL_ERRS() << "Fail "#T << LL_ENDL; \
	...
	if((std::string)test_BrowserHomePage != "http://www.AllynViewer.org") LL_ERRS() << "Fail BrowserHomePage" << LL_ENDL;
}
#endif
