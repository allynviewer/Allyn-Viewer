...
private:
	...
	boost::shared_ptr<commit_signal_t> mCommitSignal;
	boost::shared_ptr<validate_signal_t> mValidateSignal;
	bool			mIsCOA;
	bool			mIsCOAParent;
	LLControlVariable *mCOAConnectedVar;
public:
	...
	commit_signal_t* getSignal() { return mCommitSignal.get(); }
	...
	void setCOAConnect(LLControlVariable *pConnect, bool IsParent)
	{
		...
		if(!IsParent)
		{
			mCommitSignal = pConnect->mCommitSignal;
