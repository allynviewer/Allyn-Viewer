...
bool LLEnvManagerNew::useDayCycleParams(const LLSD& params, LLEnvKey::EScope scope, F32 time )
{
...
void LLEnvManagerNew::dumpPresets()
{
	...
	for (LLWLParamManager::preset_key_list_t::const_iterator it = sky_preset_keys.begin(); it != sky_preset_keys.end(); ++it)
	{
		...
		if (it->scope == LLEnvKey::SCOPE_LOCAL)
		{
...
{
	void post(LLHTTPNode::ResponsePtr response,	const LLSD& context, const LLSD& input) const
	{
