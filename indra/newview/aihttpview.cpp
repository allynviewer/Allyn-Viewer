...
class AIServiceBar : public LLView
{
  ...
  public:
	...
	void draw(void);
	LLRect getRequiredRect(void);
};
int const mc_col = number_of_capability_types;
int const bw_col = number_of_capability_types + 1;
void AIServiceBar::draw()
{
  ...
  size_t bandwidth;
  {
	...
	cts = per_service_r->mCapabilityType;
  }
