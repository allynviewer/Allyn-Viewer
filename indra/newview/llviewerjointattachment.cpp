...
void LLViewerJointAttachment::setupDrawable(LLViewerObject *object)
{
	if (!object->mDrawable)
	{
		if (mIsHUDAttachment)
		{
			log_hud_event("setupDrawable_NO_DRAWABLE", object);
		}
		return;
	}
	...
	if (mIsHUDAttachment)
	{
		log_hud_event("setupDrawable_done", object);
	}
}
BOOL LLViewerJointAttachment::addObject(LLViewerObject* object)
{
	...
	// After teleport the old HUD/attachment instance can still be listed while
	// the sim sends a new object for the same inventory item. Killing the NEW
	// object (old viewer behavior) leaves a phantom attachment: still worn,
	// but invisible until a refresh. Match Firestorm/Catznip PhantomAttach:
	// drop the stale instance and keep attaching the new one.
	if (LLViewerObject* pAttachObj = getAttachedObject(object->getAttachmentItemID()))
	{
		LL_INFOS() << "(same object re-attached)" << LL_ENDL;
		LL_INFOS("Attachment") << object->getID() << " ("<<object->getAttachmentPointName()<<") ITEM re-attached, replacing stale instance" << LL_ENDL;
		if (mIsHUDAttachment)
		{
			log_hud_event("addObject_replace_stale", pAttachObj);
			log_hud_event("addObject_keep_new", object);
		}
		pAttachObj->markDead();
		if (pAttachObj->permYouOwner() && gAgent.getRegion())
		{
	...
	if (mIsHUDAttachment)
	{
		log_hud_event(object->mDrawable ? "addObject_done" : "addObject_done_NO_DRAWABLE", object);
...
void LLViewerJointAttachment::removeObject(LLViewerObject *object)
{
	...
	if (mIsHUDAttachment)
	{
		log_hud_event("removeObject", object);
	}
