...
void init_debug_ui_menu(LLMenuGL* menu)
{
	...
	menu->addChild(new LLMenuItemCheckGL("Show search field on navigation bar", menu_toggle_control, nullptr, menu_check_control, (void*)"ShowSearchBar"));
	menu->addSeparator();
...
void init_debug_rendering_menu(LLMenuGL* menu)
{
	...
	sub_menu->addChild(new LLMenuItemCheckGL("Clouds",
	...
	menu->addChild(new LLMenuItemCheckGL("Hitboxes do avatar",
									   &menu_toggle_control,
									   nullptr,
									   &menu_check_control,
									   (void*)"DebugRenderHitboxes"));
...
class LLObjectFollow final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override
	{
		...
		if (objectp)
		{
			if (auto av = objectp->getAvatarAncestor())
				objectp = av;
...
{
	return isAgentAvatarValid() && !gAgentAvatarp->isSitting() && !gRlvHandler.hasBehaviour(RLV_BHVR_SIT);
}
...
void reset_view_final( BOOL proceed )
{
	...
	if (gRlvHandler.hasBehaviour(RLV_BHVR_CAMDISTMAX) && gRlvHandler.camPole(RLV_BHVR_CAMDISTMAX) <= 0) return;
...
class LLWorldCreateLandmark final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override
	{
		...
		create_inventory_item(gAgent.getID(), gAgent.getSessionID(),
							  ...
							  pos_string, agent_parcel->getDesc(),
...
private:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override
	{
		...
		if (selectedObject)
		{
			...
			if ( (rlv_handler_t::isEnabled()) &&
				 ( ((!index) && (gRlvAttachmentLocks.hasLockedAttachmentPoint(RLV_LOCK_ANY))) ||
				   ((index) && ((RLV_WEAR_ADD & gRlvAttachmentLocks.canAttach(attachment_point)) == 0)) ||
				   (gRlvHandler.hasBehaviour(RLV_BHVR_REZ)) ) )
			{
				setObjectSelection(nullptr);
				return true;
			}
...
void LLObjectAttachToAvatar::confirmReplaceAttachment(S32 option, LLViewerJointAttachment* attachment_point)
{
	if (option == 0)
	{
		...
		if (selectedObject)
		{
			const F32 MIN_STOP_DISTANCE = 1.f;
			const F32 ARM_LENGTH = 0.5f;
...
BOOL object_selected_and_point_valid(void *user_data)
{
	if (rlv_handler_t::isEnabled())
	{
		...
		if ( ((!pAttachPt) && (gRlvAttachmentLocks.hasLockedAttachmentPoint(RLV_LOCK_ANY))) ||
			 ((pAttachPt) && ((RLV_WEAR_ADD & gRlvAttachmentLocks.canAttach(pAttachPt)) == 0)) ||
			 (gRlvHandler.hasBehaviour(RLV_BHVR_REZ)) )
		{
			return FALSE;
		}
...
static void handle_mesh_load_obj_continued(void* data, AIFilePicker* filepicker)
{
	...
	std::string const* mesh_name = LLPolyMesh::getSharedMeshName(mesh_shared);
	LL_INFOS() << "Selected " << selected_filename << " for mesh " << *mesh_name <<LL_ENDL;
	LLFILE* fp = LLFile::fopen(selected_filename, "rb");
	if (!fp)
	{
		LL_ERRS() << "can't open: " << selected_filename << LL_ENDL;
		return;
...
{
	if (const auto& obj = objp ? objp : gObjectList.findObject(id))
		return obj->mOwnerID;
	if (const auto& obj = obj_data ? obj_data : get_obj_data(id))
		return obj->owner_id;
	return LLUUID::null;
}
const std::string get_obj_owner_slurl(const LLUUID& obj_id, const std::string& name = LLStringUtil::null, bool* group_ownedp = nullptr)
{
	...
	if (!group_ownedp)
	{
		if (const auto& obj = gObjectList.findObject(obj_id))
		{
			owner = obj->mOwnerID;
			group_owned = obj->flagObjectGroupOwned();
		}
		else if (const auto& obj = get_obj_data(obj_id))
		{
			owner = obj->owner_id;
...
const std::string get_obj_slurl(const LLUUID& id, const std::string& name = LLStringUtil::null)
{
	...
	const auto& obj = gObjectList.findObject(id);
...
class ListEnableUnmute final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override
	{
		...
		for (const auto& id : LFIDBearer::getActiveSelectedIDs())
			if (are_blocked = LLAvatarActions::isBlocked(id))
				break;
...
{
	if (ids.empty() || (choice != 1 && choice != 0)) return;
...
void initialize_menus()
{
	...
	public:
		...
		bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override
		{
			...
			gSavedSettings.setF32("CameraAngle", inst.getView());
			return true;
		}
	private:
		F32 mVal;
