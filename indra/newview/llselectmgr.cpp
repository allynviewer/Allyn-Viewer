...
void LLSelectMgr::selectionSetImage(const LLUUID& imageid)
{
	...
	struct f : public LLSelectedTEFunctor
	{
		...
		bool apply(LLViewerObject* objectp, S32 te) override
		{
			...
			{
				objectp->setTETexture(te, mImageID);
			}
			return true;
		}
	};
		...
		else
		{
			if (node->mInventorySerial != inv_serial)
			{
				if (LLViewerObject* object = node->getObject())
					if (object->getInventorySerial() != inv_serial)
			...
			if (auto obj = node->getObject()) obj->mOwnerID = owner_id;
		}
	}
	dialog_refresh_all();
	...
	if (volume)
	{
		...
		gGL.begin(LLRender::TRIANGLES);
		{
			for(S32 i = 0; i < (S32)mSilhouetteVertices.size(); i+=2)
			{
				...
				gGL.color4f(color.mV[VRED], color.mV[VGREEN], color.mV[VBLUE], 0.0f);
...
{
	return const_cast<LLSelectMgr*>(this)->mSelectedObjects->getFirstMoveableObject() != NULL;
}
