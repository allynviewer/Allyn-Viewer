										  ...
										  LLDataPacker *dp)
{
	...
	static const S32 MAX_TREE_TEXTURE_VIRTUAL_SIZE_RESET_INTERVAL = 32 ;
...
{
	const U16 FRAMES_PER_WIND_UPDATE = 20;
	...
	if (sRenderAnimateTrees)
	{
		...
		mTrunkVel += (mWind * mass_inv * TREE_WIND_SENSITIVITY);
		mTrunkVel -= (mTrunkBend * mass_inv * TREE_TRUNK_STIFFNESS);
...
void LLVOTree::setPixelAreaAndAngle(LLAgent &agent)
{
	...
	if (range < F_ALMOST_ZERO || isHUDAttachment())
	{
		mAppAngle = 180.f;
	}
	else
...
BOOL LLVOTree::updateGeometry(LLDrawable *drawable)
{
	...
	mDrawList.clear();
	if(mTrunkLOD >= sMAX_NUM_TREE_LOD_LEVELS)
	{
		mReferenceBuffer = NULL ;
	...
	{
		const F32 SRR3 = 0.577350269f;
		const F32 SRR2 = 0.707106781f;
		U32 i, j;
						 ...
						 S32 index_offset)
{
	...
	if(sRenderAnimateTrees)
	{
		...
		draw_info->mModelMatrix = new LLMatrix4a(matrix);
	...
	if (stop_level >= 0)
	{
		if (depth > stop_level)
		{
			...
			if (trunk_depth)
			{
				...
				LLMatrix4a rot_mat;
				rot_mat.setMul(trans_mat, srot_mat);
