...
const S32 MINIMUM_PRICE_FOR_LISTING = 50;
...
bool HippoGridInfo::isLindenLoginUri(const std::string& uri)
{
	if (uri.empty()) return false;
	const std::string host = utf8str_tolower(LLURI(uri).hostName());
...
{
	if (platform != PLATFORM_SECONDLIFE && isLindenLoginUri(mLoginUri))
	{
		platform = PLATFORM_SECONDLIFE;
	}
	mPlatform = platform;
...
void HippoGridInfo::setLoginUri(const std::string& loginUri)
{
	...
	const std::string host = utf8str_tolower(LLURI(mLoginUri).hostName());
	if (host == "login.agni.lindenlab.com")
	{
		mIsInProductionGrid = true;
		useHttps();
		setPlatform(PLATFORM_SECONDLIFE);
	}
	else if (isLindenLoginUri(mLoginUri))
	{
		useHttps();
		setPlatform(PLATFORM_SECONDLIFE);
	}
	else if (host == "login.avination.com" || host == "login.avination.net")
	{
		mIsInAvination = true;
		useHttps();
	}
...
void HippoGridManager::deleteGrid(const std::string& grid)
{
	...
	if (grid == mDefaultGrid)
		setDefaultGrid(LLStringUtil::null);
