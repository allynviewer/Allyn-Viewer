...
U64MicrosecondsImplicit	gStartTime = 0;
U32 gFrameStalls = 0;
...
const std::string MARKER_FILE_NAME("Allyn.exec_marker");
const std::string START_MARKER_FILE_NAME("Allyn.start_marker");;
const std::string ERROR_MARKER_FILE_NAME("Allyn.error_marker");
const std::string LLERROR_MARKER_FILE_NAME("Allyn.llerror_marker");
const std::string LOGOUT_MARKER_FILE_NAME("Allyn.logout_marker");
const std::string LOG_FILE("Allyn.log");
extern const std::string OLD_LOG_FILE("Allyn.old");
...
{
	default_trans_args.insert("SECOND_LIFE");
...
static void settings_to_globals()
{
	...
	NAV_BAR_HEIGHT		= gSavedSettings.getS32("NavigationBarHeight");
	if (NAV_BAR_HEIGHT <= 0)
	{
		NAV_BAR_HEIGHT = 29;
	}
	...
	mAgentRegionLastAlive(false)
{
	...
	}
	{
		...
		gDirUtilp->initAppDirs("AllynViewer", newview_path);
	}
	sInstance = this;
	...
	try
	{
		while (!LLApp::isExiting())
		{
			...
			pauseMainloopTimeout();
			{
				...
				const F64 max_idle_time = run_multiple_threads ? 0.0 : llmin(.005*10.0*gFrameIntervalSeconds.value(), 0.005);
				idleTimer.reset();
				while(1)
				{
					S32 work_pending = 0;
					S32 io_pending = 0;
					{
						...
						work_pending += LLAppViewer::getTextureCache()->update(1);
					}
					{
						LL_RECORD_BLOCK_TIME(FTM_DECODE);
						work_pending += LLAppViewer::getImageDecodeThread()->update(1);
					}
					{
						LL_RECORD_BLOCK_TIME(FTM_DECODE);
						work_pending += LLAppViewer::getTextureFetch()->update(1);
					}
					{
...
bool LLAppViewer::initThreads()
{
	...
	if (gSavedSettings.getU32("TexturePipelineVersion") < 1)
	{
		if (gSavedSettings.getF32("HTTPThrottleBandwidth") <= 3000.f)
		{
			gSavedSettings.setF32("HTTPThrottleBandwidth", 15000.f);
		}
		if (gSavedSettings.getU32("CacheSize") < 1024)
		{
			gSavedSettings.setU32("CacheSize", 2048);
		}
		if (gSavedSettings.getS32("TextureFetchUpdateHighPriority") <= 32)
		{
			gSavedSettings.setS32("TextureFetchUpdateHighPriority", 64);
		}
		if (gSavedSettings.getS32("TextureFetchUpdatePriorities") <= 32)
		{
			gSavedSettings.setS32("TextureFetchUpdatePriorities", 64);
		}
		if (gSavedSettings.getS32("TextureFetchUpdateMinMediumPriority") <= 32)
		{
			gSavedSettings.setS32("TextureFetchUpdateMinMediumPriority", 64);
		}
		if (gSavedSettings.getS32("TextureFetchUpdateMaxMediumPriority") <= 256)
		{
			gSavedSettings.setS32("TextureFetchUpdateMaxMediumPriority", 512);
		}
		gSavedSettings.setU32("TexturePipelineVersion", 1);
		LL_INFOS() << "Applied texture pipeline defaults: HTTP throttle "
				   << gSavedSettings.getF32("HTTPThrottleBandwidth")
				   << " kbps, cache " << gSavedSettings.getU32("CacheSize") << " MB" << LL_ENDL;
	}
	startEngineThread();
	...
	S32 decode_threads = 1;
	if (enable_threads)
	{
		S32 cores = (S32)std::thread::hardware_concurrency();
		if (cores <= 0)
		{
			cores = 4;
		}
		decode_threads = llclamp(cores - 4, 2, 8);
		U32 override_threads = gSavedSettings.getU32("ImageDecodeThreads");
		if (override_threads > 0)
		{
			decode_threads = llclamp((S32)override_threads, 1, 16);
		}
	}
	LL_INFOS() << "Starting image decode with " << decode_threads << " threads" << LL_ENDL;
	LLAppViewer::sImageDecodeThread = new LLImageDecodeThread(enable_threads && true, llmax(decode_threads - 1, 0));
...
void LLAppViewer::initLoggingInternal()
{
	...
	llstat exec_marker_stat;
	std::string exec_marker_file_name = gDirUtilp->getExpandedFilename(LL_PATH_LOGS, MARKER_FILE_NAME);
	std::ostringstream duration_log_stream;
	int start_stat_result = LLFile::stat(start_marker_file_name, &start_marker_stat);
	int exec_stat_result = LLFile::stat(exec_marker_file_name, &exec_marker_stat);
	if ( 0 == start_stat_result && 0 == exec_stat_result )
	{
		int elapsed_seconds = exec_marker_stat.st_ctime - start_marker_stat.st_ctime;
	...
	{
		duration_log_stream << "duration stat failure; start: "<< start_stat_result << " exec: " << exec_stat_result;
		gLastExecDuration = -1;
	}
...
bool LLAppViewer::initConfiguration()
{
	...
	if (clp.hasOption("portable"))
	{
		...
		initLoggingInternal();
		LL_INFOS() << "Portable viewer configuration initialized (crash-only logging)!" << LL_ENDL;
	}
...
void LLAppViewer::cleanupSavedSettings()
{
	...
	gSavedSettings.setBOOL("UseEnergy", TRUE);
...
void LLAppViewer::requestQuit()
{
	...
	{
		gAgentAvatarp->updateAvatarRezMetrics(true);
	}
	...
	gLogoutTimer.reset();
	mQuitRequested = true;
	if (gViewerWindow && gViewerWindow->getProgressView()
...
static void purgeCefCachesOnStartup()
{
	const std::string cache_root = gDirUtilp->getCacheDir(false);
	const std::string cef_cache = gDirUtilp->add(cache_root, "cef_cache");
	const std::string cef_cache_old = cef_cache + "_old";
	const std::string cef_data = gDirUtilp->add(cache_root, "cef_data");

	if (LLFile::isdir(cef_cache_old))
	{
		gDirUtilp->deleteDirAndContents(cef_cache_old);
	}

	if (LLFile::isdir(cef_cache))
	{
		if (LLFile::rename(cef_cache, cef_cache_old) == 0)
		{
			std::thread([cef_cache_old]() {
				gDirUtilp->deleteDirAndContents(cef_cache_old);
			}).detach();
		}
		else
		{
			LLDirIterator iter(cef_cache, "*");
			std::string name;
			while (iter.next(name))
			{
				...
				const std::string src = gDirUtilp->add(cef_cache, name);
				if (!LLFile::isdir(src))
				{
					continue;
				}
				const std::string dst = src + "_old";
				if (LLFile::rename(src, dst) == 0)
				{
					std::thread([dst]() { gDirUtilp->deleteDirAndContents(dst); }).detach();
				}
			}
		}
	}

	if (LLFile::isdir(cef_data))
	{
		gDirUtilp->deleteDirAndContents(cef_data);
	}
}
...
{
	static const U32 TEXTURE_CACHE_VERSION = 9;
...
#define LAZY_FT(str) static LLTrace::BlockTimerStatHandle ftm(str); LL_RECORD_BLOCK_TIME(ftm)
	...
	if (gTeleportDisplay && !hide_tp_screen)
	{
		if (gAgent.getTeleportState() == LLAgent::TELEPORT_ARRIVING)
		{
			gVLManager.unpackData();
			const F32 max_region_update_time = .001f;
	...
	if (!gNoRender)
	{
		...
		regionp = LLWorld::getInstance()->resolveRegionGlobal(wind_position_region, gAgent.getPositionGlobal());
		if (regionp)
		{
	...
	if (LLStartUp::getStartupState() == STATE_STARTED)
	{
		static LLFrameTimer scene_load_timer;
		if (scene_load_timer.getElapsedTimeF32() >= 5.f)
		{
			scene_load_timer.reset();
			LLViewerRegion* regionp = gAgent.getRegion();
			LLTextureFetch* fetchp = getTextureFetch();
			LL_INFOS("SceneLoad")
				<< "region=" << (regionp ? regionp->getName() : std::string("<none>"))
				<< " caps=" << (regionp && regionp->capabilitiesReceived() ? "yes" : "no")
				<< " GetTexture=" << (regionp ? regionp->getCapability("GetTexture") : std::string())
				<< " ViewerAsset=" << (regionp ? regionp->getViewerAssetUrl() : std::string())
				<< " GetMesh2=" << (regionp ? regionp->getCapability("GetMesh2") : std::string())
				<< LL_ENDL;
			LL_INFOS("SceneLoad")
				<< "objects total=" << gObjectList.getNumObjects()
				<< " active=" << gObjectList.getNumActiveObjects()
				<< " new=" << gObjectList.mNumNewObjects
				<< " orphans=" << gObjectList.getOrphanCount()
				<< " obj_kb=" << F64Kilobytes(gTotalObjectData).value()
				<< LL_ENDL;
			LL_INFOS("SceneLoad")
				<< "terrain land_pkts=" << gVLManager.getLandPackets()
				<< " water_pkts=" << gVLManager.getWaterPackets()
				<< " land_kb=" << (gVLManager.getLandBits() / 8192)
				<< " world_bytes=" << gVLManager.getTotalBytes()
				<< LL_ENDL;
			if (fetchp)
			{
				LL_INFOS("SceneLoad")
					<< "tex images=" << gTextureList.getNumImages()
					<< " http_ok=" << fetchp->getHttpOkCount()
					<< " http_206=" << fetchp->getHttpPartialCount()
					<< " http_403=" << fetchp->getHttpForbiddenCount()
					<< " http_404=" << fetchp->getHttpNotFoundCount()
					<< " http_other=" << fetchp->getHttpOtherFailCount()
					<< " map_fail=" << fetchp->getHttpMapFailCount()
					<< " pending=" << fetchp->getNumRequests()
					<< " http_pending=" << fetchp->getNumHTTPRequests()
					<< " tex_http_kb=" << (fetchp->getHttpTextureBytes() / 1024)
					<< " tex_total_kb=" << F64Kilobytes(gTotalTextureData).value()
					<< LL_ENDL;
			}
			LL_INFOS("SceneLoad")
				<< "mesh recv_kb=" << (LLMeshRepository::sBytesReceived / 1024)
				<< " http=" << LLMeshRepository::sHTTPRequestCount
				<< " retry=" << LLMeshRepository::sHTTPRetryCount
				<< " lod_pend=" << LLMeshRepository::sLODPending
				<< " lod_proc=" << LLMeshRepository::sLODProcessing
				<< LL_ENDL;
		}
	}
	stop_glerror();
...
{
	const bool skip_ui_wait = gTeleportDisplay
		|| (gViewerWindow && gViewerWindow->getProgressView()
			&& gViewerWindow->getProgressView()->getVisible());
	if (!skip_ui_wait && LLModalDialog::activeCount() > 0)
	{
		return;
	}
	if(gIMMgr)
...
void LLAppViewer::idleNetwork()
{
	...
	LLEventPoll::dispatchPending();
	gObjectList.mNumNewObjects = 0;
	S32 total_decoded = 0;
	static const LLCachedControl<bool> speedTest(gSavedSettings, "SpeedTest");
	if (!speedTest)
	{
		LL_RECORD_BLOCK_TIME(FTM_IDLE_NETWORK);
		LL_PUSH_CALLSTACKS();
		LLTimer check_message_timer;
		const S64 frame_count = gFrameCount;
		F32 total_time = 0.0f;
	...
	if (agent_region && (LLStartUp::getStartupState()==STATE_STARTED))
	{
		...
		if ((mAgentRegionLastAlive && !this_region_alive)
...
void LLAppViewer::idleAudio()
{
	...
	if (gAudiop)
	{
		...
		const F32 max_audio_decode_time = 0.002f;
