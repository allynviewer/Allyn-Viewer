...
const std::string LLAutoReplaceSettings::AUTOREPLACE_LIST_NAME         = "name";
const std::string LLAutoReplaceSettings::AUTOREPLACE_LIST_REPLACEMENTS = "replacements";
...
LLSD LLAutoReplaceSettings::getListNames()
{
		...
		)
	{
		...
		{
			LL_WARNS("AutoReplace")<<counter<<" ! not a map: "<< LL_ENDL;
		}
		counter++;
	}
	LL_DEBUGS("AutoReplace")<<"^^^^^^"<<LL_ENDL;
...
bool LLAutoReplaceSettings::decreaseListPriority(std::string listName)
{
	...
	if (found_index != -1)
	{
		...
		{
			LL_WARNS("AutoReplace")<<next_index<<" ! not a map: "<< LL_ENDL;
		}
...
{
	std::string returnedWord = currentWord;
