...
void LLFloaterCustomize::setCurrentWearableType( LLWearableType::EType type, bool disable_camera_switch )
{
	if( mCurrentWearableType != type )
	{
		...
		else if(!gSavedSettings.getBOOL("AppearanceCameraMovement") || disable_camera_switch)
		{
...
void LLFloaterCustomize::onBtnImport_continued(AIFilePicker* filepicker)
{
	...
	bool different_grid = false;
	bool mixed_grids = false;
	std::string gridnick;
	std::string wearable_types;
	try
	{
		std::string metaversion;
		U32 metaversion_major = 0;
...
void LLFloaterCustomize::onBtnExport()
{
	...
	if (old_wearable)
	{
		...
		{
			can_export = item->getPermissions().allowExportBy(gAgent.getID(), LFSimFeatureHandler::instance().exportPolicy());
		}
	}
	if (!can_export)
	{
		LL_WARNS() << "Cannot export current wearable \"" << name << "\" of type " << (int)edit_type << " because the user is not its creator or lacks export permission." << LL_ENDL;
		return;
	}
...
{
	if (val && LLAppearanceMgr::instance().isOutfitDirty())
		LLAppearanceMgr::instance().updateBaseOutfit();
	else
		new LLMakeOutfitDialog(true);
}
...
void LLFloaterCustomize::draw()
{
	...
	updateInventoryUI();
	if (isAgentAvatarValid() && mMetricHeight && mImperialHeight)
	{
...
void LLFloaterCustomize::updateVisiblity(bool force_disable_camera_switch)
{
	if(!getVisible())
	{
...
bool LLFloaterCustomize::onSaveDialog(const LLSD& notification, const LLSD& response )
{
	...
	mNextStepAfterSaveCallback.disconnect_all_slots();
	return false;
}
