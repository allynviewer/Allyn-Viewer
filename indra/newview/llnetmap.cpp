...
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
...
class mmsetred : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
class mmsetgreen : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
class mmsetblue : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
class mmsetyellow : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
class mmsetcustom : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
class mmsetunmark : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
class mmenableunmark : public view_listener_t
{
public:
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata) override final;
};
...
const F32 MAP_SCALE_ZOOM_FACTOR = 1.1f;
...
std::map<LLUUID, LLVector3d>	LLNetMap::mClosestAgentsToCursor;
uuid_vec_t						LLNetMap::mClosestAgentsAtLastClick;
LLNetMap::LLNetMap(const std::string& name) :
	LLPanel(name),
	mScale(128.f),
	mObjectMapTPM(1.f),
	mObjectMapPixels(255.f),
	mPickRadius(gSavedSettings, "ExodusMinimapAreaEffect"),
	mTargetPan(0.f, 0.f),
	mCurPan(0.f, 0.f),
	mStartPan(0.f, 0.f),
	mMouseDown(0, 0),
...
BOOL LLNetMap::handleToolTip( S32 x, S32 y, std::string& tool_tip, LLRect* sticky_rect_screen )
{
	...
	if( region )
	{
		...
		if (region->mMapAvatarIDs.size())
		{
			...
			{
				bool single_agent(mClosestAgentsToCursor.size() == 1);
				if (!single_agent)
