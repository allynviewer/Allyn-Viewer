	...
	mPanel(NULL)
{
	...
	LLUICtrlFactory::getInstance()->buildFloater(this, "floater_chat_history.xml", &getFactoryMap(), false);
...
{
	gConsole->setVisible( !isInVisibleChain()
							|| isMinimized()
...
void LLFloaterChat::addChatHistory(LLChat& chat, bool log_to_file)
{
	...
	if (!log_to_file) color = gSavedSettings.getColor("LogChatColor");
			  ...
			  BOOL local_agent)
{
		...
		&& !local_agent)
	{
		...
		{
			std::string chit(chat.mText),
						chat;
			LLUrlMatch match;
			while (!chit.empty() && LLUrlRegistry::instance().findUrl(chit, match))
			{
				const auto start(match.getStart()), length(match.getEnd()+1-start);
				if (start > 0) chat += chit.substr(0, start);
				chat += match.getLabel() + match.getQuery();
				chit.erase(0, start+length);
			}
			if (!chit.empty()) chat += chit;
