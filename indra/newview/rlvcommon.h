...
public:
	...
	static const std::string& getAnonym(const LLAvatarName& avName);
	static const std::string& getAnonym(const std::string& strName);
	...
	static std::string        getVersion(bool fLegacy = false);
...
public:
	...
	static bool isNearbyAgent(const LLUUID& idAgent);
	static bool isNearbyRegion(const std::string& strRegion);
	static void filterLocation(std::string& strUTF8Text);
	static void filterNames(std::string& strUTF8Text, bool fFilterLegacy = true);
	static void filterScriptQuestions(S32& nQuestions, LLSD& sdPayload);
	static bool isForceTp()	{ return m_fForceTp; }
	static void forceTp(const LLVector3d& posDest);
