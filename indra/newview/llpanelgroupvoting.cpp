...
public:
	...
	AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return startGroupVoteResponder_timeout; }
	char const* getName(void) const { return "LLStartGroupVoteResponder"; }
private:
	LLUUID mGroupID;
};
...
public:
	...
	AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return groupProposalBallotResponder_timeout; }
	char const* getName(void) const { return "LLGroupProposalBallotResponder"; }
private:
	LLUUID mGroupID;
};
void LLPanelGroupVoting::impl::sendStartGroupProposal()
{
	if ( !gAgent.hasPowerInGroup(mGroupID, GP_PROPOSAL_START) )
		return;
	F32 majority = 0.f;
	S32 duration_seconds = (S32)mDuration->get()*24*60*60;
