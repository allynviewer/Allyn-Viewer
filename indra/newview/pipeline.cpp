...
bool addDeferredAttachments(LLRenderTarget& target)
{
	return target.addColorAttachment(GL_SRGB8_ALPHA8) &&
			target.addColorAttachment(GL_RGB10_A2);
}
...
void LLPipeline::init()
{
	...
	{
		setAllRenderDebugFeatures();
	}
	clearAllRenderDebugDisplays();
...
LLPipeline::eFBOStatus LLPipeline::doAllocateScreenBuffer(U32 resX, U32 resY)
{
	...
	U32 samples = RenderFSAASamples.get() - RenderFSAASamples.get() % 2;
...
bool LLPipeline::allocateScreenBuffer(U32 resX, U32 resY, U32 samples)
{
	...
	if (LLPipeline::sRenderDeferred)
	{
		...
		if (shadow_detail > 0)
		{
			U32 sun_shadow_map_width = ((U32(resX*scale)+1)&~1);
			for (U32 i = 0; i < 4; i++)
			{
...
bool LLPipeline::isRenderDeferredCapable()
{
	return gGLManager.mHasFramebufferObject &&
		...
		LLFeatureManager::getInstance()->isFeatureAvailable("RenderAvatarVP") &&
...
void LLPipeline::refreshCachedSettings()
{
	...
	LLVertexBuffer::sUseVAO = gSavedSettings.getBOOL("RenderUseVAO") && gSavedSettings.getBOOL("VertexShaderEnable");
	LLVertexBuffer::sDisableVBOMapping = LLVertexBuffer::sEnableVBOs;
...
void LLPipeline::createObject(LLViewerObject* vobj)
{
	...
	if (vobj->getParent())
	{
		vobj->setDrawableParent(((LLViewerObject*)vobj->getParent())->mDrawable);
	}
	else
	{
...
void render_hud_elements()
{
	...
	if (!LLPipeline::sReflectionRender && gPipeline.hasRenderDebugFeatureMask(LLPipeline::RENDER_DEBUG_FEATURE_UI))
	{
		...
		gViewerWindow->renderSelections(FALSE, FALSE, FALSE);
		LLTracker::render3D();
...
void LLPipeline::renderGeom(LLCamera& camera, BOOL forceVBOUpdate)
{
	...
	}
	{
		...
		if (!gPipeline.hasRenderType(LLPipeline::RENDER_TYPE_HUD))
		{
			...
			if (!LLGLState<GL_LIGHTING>::checkEnabled())
			{
				LL_WARNS_ONCE("PIPELINE") << "GL_LIGHTING not enabled after updateHWLightMode" << LL_ENDL;
				gGL.flush();
				static_cast<void>(glEnable(GL_LIGHTING));
				LLGLState<GL_LIGHTING>::staticData.currentState = true;
			}
		}
...
void LLPipeline::rebuildPools()
{
	...
	while(max_count > 0 && mPools.size() > 0)
	{
		if (iter1 == mPools.end())
		{
			iter1 = mPools.begin();
...
U8 LLPipeline::setupFeatureLights(U8 cur_count)
{
	if (mLightMode == LIGHT_MODE_EDIT)
	{
		...
		LLVector4a light_pos_cam(-8.f, 0.25f, 10.f, 0.f);
	...
	else if (mLightMode == LIGHT_MODE_BACKLIGHT)
	{
	...
	if (isLocalLightingEnabled())
	{
			 ...
			 iter != mNearbyLights.end(); ++iter)
		{
			...
			if (light->isLightSpotlight()
			    && (LLPipeline::sRenderDeferred || RenderSpotLightsInNondeferred))
			{
				LLQuaternion quat = light->getRenderRotation();
				LLVector3 at_axis(0,0,-1);
				at_axis *= quat;
...
void LLPipeline::enableLightsDynamic(LLGLState<GL_LIGHTING>& light_state)
{
	...
	{
		if (gAgentAvatarp->mSpecialRenderMode == 0)
		{
			gPipeline.enableLightsAvatar(light_state);
			return;
		}
		else if (gAgentAvatarp->mSpecialRenderMode >= 1)
		{
...
{
	updateHWLightMode(gAvatarBacklight ? LIGHT_MODE_BACKLIGHT : LIGHT_MODE_NORMAL);
	enableLights(0xFFFFFFFF, light_state);
}
...
void LLPipeline::renderBloom(BOOL for_snapshot, F32 zoom_factor, int subfield, bool tiling)
{
	...
	gGL.setColorMask(true, true);
	glClearColor(0,0,0,0);
	if (tiling && !LLPipeline::sRenderDeferred)
	{
		...
		gGL.getTexUnit(0)->bind(&mGlow[1]);
		{
			...
			llassert(zoom_factor > 0.0);
			const F32 tile_size = 1.0/zoom_factor;
			LLVector2 tc1 = tile*tile_size;
			LLVector2 tc2 = (tile+LLVector2(1,1))*tile_size;
			LLGLEnable<GL_BLEND> blend;
...
void LLPipeline::bindDeferredShader(LLGLSLShader& shader, LLRenderTarget* diffuse_source, LLRenderTarget* light_source)
{
	...
	if (channel > -1 || channel2 >= -1)
	{
		...
		if(channel2 > -1)
		{
			...
			if(scale < 1.f)
				...
				gGL.getTexUnit(channel2)->bind(&mDeferredDepth, TRUE);
		}
		stop_glerror();
		stop_glerror();
	...
	shader.uniform1f (LLShaderMgr::DEFERRED_SHADOW_OFFSET, RenderShadowOffset);
...
void LLPipeline::renderDeferredLighting()
{
	...
	static const LLCachedControl<bool> RenderLocalLights("RenderLocalLights",false);
	{
		...
		if (!LLGLState<GL_LIGHTING>::checkEnabled())
		{
			LL_WARNS_ONCE("PIPELINE") << "GL_LIGHTING not enabled before deferred SSAO path" << LL_ENDL;
			gGL.flush();
			static_cast<void>(glEnable(GL_LIGHTING));
			LLGLState<GL_LIGHTING>::staticData.currentState = true;
		}
...
void LLPipeline::generateImpostor(LLVOAvatar* avatar, bool preview_avatar)
{
	...
	BOOL old_use_impostors = LLVOAvatar::sUseImpostors;
	{
		...
		for(;attachment_iter != end;++attachment_iter)
		{
			...
			if (!attached_object || attached_object->isDead() || !attached_object->mDrawable)
			{
				continue;
			}
			if (preview_avatar && attached_object->getControlAvatar())
			{
				continue;
			}
			if (LLSpatialBridge* bridge = attached_object->mDrawable->getSpatialBridge())
			{
				markVisible(bridge, *viewer_camera);
			}
		}
	}
	...
	if (preview_avatar)
	{
		LLVOAvatar::sUseImpostors = old_use_impostors;
	}
	else
	{
