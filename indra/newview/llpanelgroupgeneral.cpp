...
BOOL LLPanelGroupGeneral::postBuild()
{
	...
	if (mGroupID.isNull())
	{
		...
		getChildView("copy_uri")->setVisible(false);
...
void LLPanelGroupGeneral::update(LLGroupChange gc)
{
	...
	if(gc == GC_ALL || gc == GC_PROPERTIES)
	{
		if (mCtrlReceiveNotices)
		{
			...
			if (is_member)
			{
				if(!mCtrlReceiveNotices->isDirty())
				{
...
void LLPanelGroupGeneral::addMember(LLGroupMemberData* member)
{
	...
	item_params.columns.add().column("name").font("SANSSERIF_SMALL").font_style(style);
	item_params.columns.add().column("title").value(member->getTitle()).font("SANSSERIF_SMALL").font_style(style);
	...
	item_params.columns.add().column("online").value(member->getOnlineStatus())
		...
		.font("SANSSERIF_SMALL").font_style(style);
	mListVisibleMembers->addNameItemRow(item_params);
}
