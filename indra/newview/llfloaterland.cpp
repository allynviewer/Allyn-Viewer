...
:	LLFloater(std::string("floaterland"), std::string("FloaterLandRect6"), std::string("About Land"))
	, mTabLand(nullptr)
...
BOOL LLFloaterLand::postBuild()
{
	...
	if (tab)
	{
		if (!gAgent.getRegion()->getCapability("RegionExperiences").empty())
			createPanelLandExperiences(this);
...
void* LLFloaterLand::createPanelLandExperiences(void* data)
{
	...
	self->mTabLand->addTabPanel(self->mPanelExperiences, self->mPanelExperiences->getLabel());
...
void LLPanelLandOptions::refresh()
{
	...
	else
	{
		...
		mCheckEditGroupObjects	->setEnabled( can_change_options && !parcel->getAllowModify() );
...
void LLPanelLandOptions::onCommitAny()
{
	...
	if (region && region->getAllowDamage())
	{
		...
		{
			mCheckOtherScripts->set(parcel->getParcelFlag(PF_ALLOW_OTHER_SCRIPTS));
...
BOOL LLPanelLandAccess::postBuild()
{
	...
	if (mListAccess)
	{
		mListAccess->sortByColumnIndex(0, TRUE);
	}
	mListBanned = getChild<LLNameListCtrl>("BannedList");
	if (mListBanned)
	{
		mListBanned->sortByColumnIndex(0, TRUE);
	}
	return TRUE;
}
