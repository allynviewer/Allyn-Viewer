...
public:
	...
	S32 getNumDeferredPasses() { return 1; }
	void beginDeferredPass(S32 pass);
	void endDeferredPass(S32 pass);
	void renderDeferred(S32 pass);
	S32 getNumShadowPasses() { return 1; }
	void beginShadowPass(S32 pass);
	void endShadowPass(S32 pass);
	void renderShadow(S32 pass);
	void render(S32 pass = 0);
	void prerender();
	void beginRenderPass( S32 pass );
	void endRenderPass( S32 pass );
	void dirtyTextures(const std::set<LLViewerFetchedTexture*>& textures);
