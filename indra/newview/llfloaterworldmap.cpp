...
void LLFloaterWorldMap::trackLandmark( const LLUUID& landmark_item_id )
{
	...
	if (found && iface->setCurrentByID( landmark_item_id ) )
	{
		...
		LLTracker::trackLandmark(mLandmarkAssetIDList.at( idx ),
								mLandmarkItemIDList.at( idx ),
								name);
		if( asset_id != sHomeID )
		{
...
void LLFloaterWorldMap::updateTeleportCoordsDisplay( const LLVector3d& pos )
{
	...
	F32 region_local_z = (F32)llclamp( pos.mdV[VZ], 0.0, 8192.0 );
...
{
	std::string name;
	trackAvatar( id, gCacheName->getFullName(id, name) ? name : "av" );
	onShowTargetBtn();
}
...
{
	LLWorldMapView::setPan( 0, 0, FALSE);
	mSetToUserPosition = TRUE;
}
