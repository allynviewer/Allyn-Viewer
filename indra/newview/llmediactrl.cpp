...
LLMediaCtrl::LLMediaCtrl( const Params& p) :
	...
	mIgnoreUIScale( false ),
	mPageZoomOverride( -1.f ),
	mContentScale( 1.f ),
	mHomePageUrl( "" ),
	...
	mContextMenu()
{
	...
	if(!getDecoupleTextureSize())
	{
		...
		S32 screen_width = llmax(64, ll_round((F32)getRect().getWidth() * sx));
		S32 screen_height = llmax(64, ll_round((F32)getRect().getHeight() * sy));
...
void LLMediaCtrl::reshape( S32 width, S32 height, BOOL called_from_parent )
{
	if(!getDecoupleTextureSize())
	{
		F32 sx, sy;
		getTextureScale(sx, sy);
		S32 screen_width = llmax(64, ll_round((F32)width * sx));
		S32 screen_height = llmax(64, ll_round((F32)height * sy));
		if ( screen_height > 0 && screen_width > 0
			&& (screen_width != mTextureWidth || screen_height != mTextureHeight) )
		{
			setTextureSize(screen_width, screen_height);
		}
	}
...
void LLMediaCtrl::setPageZoomOverride(F32 zoom)
{
	if (mPageZoomOverride == zoom)
	{
		return;
	}
	mPageZoomOverride = zoom;
	if (mMediaSource && zoom >= 0.f)
	{
		mMediaSource->setPageZoomFactor(zoom);
		mUpdateScrolls = true;
	}
}
void LLMediaCtrl::setIgnoreUIScale(bool ignore)
{
	if (mIgnoreUIScale == ignore)
	{
		return;
	}
	mIgnoreUIScale = ignore;
	const LLRect& r = getRect();
	reshape(r.getWidth(), r.getHeight(), FALSE);
}
void LLMediaCtrl::setContentScale(F32 scale)
{
	scale = llclamp(scale, 0.5f, 3.f);
	if (mContentScale == scale)
	{
		return;
	}
	mContentScale = scale;
...
void LLMediaCtrl::getTextureScale(F32& sx, F32& sy) const
{
	if (mIgnoreUIScale)
	{
		sx = sy = 1.f / mContentScale;
		return;
	}
	const LLVector2 scale = LLUI::getScaleFactor();
	sx = scale.mV[VX] / mContentScale;
	sy = scale.mV[VY] / mContentScale;
}
...
bool LLMediaCtrl::ensureMediaSourceExists()
{
	if(mMediaSource.isNull())
	{
		...
		if ( mMediaSource )
		{
			...
			if (!mIgnoreUIScale)
			{
				F32 scale_factor = (mPageZoomOverride >= 0.f) ? mPageZoomOverride : LLUI::getScaleFactor().mV[ VX ];
...
void LLMediaCtrl::convertInputCoords(S32& x, S32& y)
{
	...
	{
		y = ll_round((F32)(getRect().getHeight() - y) * sy);
	};
}
