	...
	if (item)
	{
		if (LLLandmark* lm = gLandmarkList.getAsset(item->getAssetUUID()))
		{
			if (LLViewerRegion* region = gAgent.getRegion())
				use_backup = !lm->getRegionID(id) || id == region->getRegionID();
			if (!use_backup)
			{
...
,	mName(key.has("NAME") ? key["NAME"].asString() : gAgent.getRegion()->getName())
,	mShakeIterations()
,	mShakeMagnitude()
{
...
{
	if (sShakeState != SHAKE_DONE && sShakeState != SHAKE_START)
	{
...
void LLFloaterRegionRestarting::draw()
{
	...
	if (!alchemyRegionShake || isMinimized())
		return;
	const F32 SHAKE_INTERVAL = 0.025f;
	const F32 SHAKE_TOTAL_DURATION = 1.8f;
	...
	if (SHAKE_DONE != sShakeState && mShakeTimer.hasExpired())
	{
		...
		switch(sShakeState)
		{
			...
			case SHAKE_DOWN:
				...
				mShakeIterations++;
				time_shaking = SHAKE_INTERVAL * (mShakeIterations * 4 );
				...
				else
				{
					...
					mShakeMagnitude = SHAKE_INITIAL_MAGNITUDE * (percent_done_shaking * percent_done_shaking);
				}
				break;
			default:
				break;
