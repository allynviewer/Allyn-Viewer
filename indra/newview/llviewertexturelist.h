...
private:
	...
	void maybeAddToFastCache(LLViewerFetchedTexture* imagep, LLViewerTexture::EBoostLevel boost_priority);
...
{
public:
	LLPointer<LLUIImage> getUIImageByID(const LLUUID& id, S32 priority);
	LLPointer<LLUIImage> getUIImage(const std::string& name, S32 priority);
	void cleanUp();
	bool initFromFile();
