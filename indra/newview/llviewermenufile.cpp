...
class LLFileEnableSaveAs : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		if (frontmost && frontmost->hasChild("Save Preview As...", true))
		{
...
class LLFileUploadImage : public view_listener_t, public AIFileUpload
{
  ...
  protected:
	void handle_event(std::string const& filename)
	{
			 ...
			 void *userdata)
{
	...
	else if(exten == "tmp")
	{
         ...
         LLFILE* in = LLFile::fopen(src_filename, "rb");
         if (in)
         {
                 char buf[16384];
                 size_t readbytes;
                 ...
                 LLFILE* out = LLFile::fopen(filename, "wb");
				 created_temp_file = true;
                 if (out)
                 {
	...
	if (!error)
	{
		...
		upload_new_resource(tid, asset_type, name, desc, compression_info,
