...
BOOL LLPrefsVoice::postBuild()
{
	...
	getChild<LLUICtrl>("device_settings_btn")->setCommitCallback(boost::bind(LLFloaterVoiceDeviceSettings::showInstance));
	...
	if (LLCheckBoxCtrl* check = findChild<LLCheckBoxCtrl>("enable_multivoice_check"))
	{
		...
		std::string multivoice_label;
		if (hasString("multivoice_label"))
		{
			multivoice_label = getString("multivoice_label", LLTrans::getDefaultArgs());
		}
		else if (LLPanel* inner = findChild<LLPanel>("voice_scroll_inner"))
		{
			if (inner->hasString("multivoice_label"))
			{
				multivoice_label = inner->getString("multivoice_label", LLTrans::getDefaultArgs());
			}
		}
		if (!multivoice_label.empty())
		{
			check->setLabel(multivoice_label);
		}
	}
...
void LLPrefsVoice::apply()
{
	...
	const bool needs_vivox_license =
		(gSavedSettings.getString("VoiceServerType") == "vivox") &&
		!gSavedSettings.getBOOL("VivoxLicenseAccepted");
	if (enable_voice && needs_vivox_license)
	{
