...
public:
	...
	AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return baseCapabilitiesComplete_timeout; }
	char const* getName(void) const { return "BaseCapabilitiesComplete"; }
private:
    ...
    void httpSuccess(void)
    {
		...
		if( mID != regionp->getHttpResponderID() )
		{
...
public:
	...
	AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return baseCapabilitiesCompleteTracker_timeout; }
	char const* getName(void) const { return "BaseCapabilitiesCompleteTracker"; }
private:
	void httpFailure()
	{
	...
	mWidth(region_width_meters)
{
	...
	initStats();
	initPartitions();
	setCapabilitiesReceivedCallback(boost::bind(&LLAgent::handleServerBakeRegionTransition, &gAgent, _1));
}
void LLViewerRegion::initPartitions()
{
	mImpl->mObjectPartition.push_back(new LLHUDPartition(this));
	mImpl->mObjectPartition.push_back(new LLTerrainPartition(this));
	mImpl->mObjectPartition.push_back(new LLVoidWaterPartition(this));
	mImpl->mObjectPartition.push_back(new LLWaterPartition(this));
	mImpl->mObjectPartition.push_back(new LLTreePartition(this));
	mImpl->mObjectPartition.push_back(new LLParticlePartition(this));
#if ENABLE_CLASSIC_CLOUDS
	mImpl->mObjectPartition.push_back(new LLCloudPartition(this));
#endif
	mImpl->mObjectPartition.push_back(new LLGrassPartition(this));
	mImpl->mObjectPartition.push_back(new LLVolumePartition(this));
	mImpl->mObjectPartition.push_back(new LLBridgePartition(this));
	mImpl->mObjectPartition.push_back(new LLAttachmentPartition(this));
	mImpl->mObjectPartition.push_back(new LLHUDParticlePartition(this));
	mImpl->mObjectPartition.push_back(NULL);
	mRenderInfoRequestTimer.resetWithExpiry(0.f);
...
public:
	...
	AIHTTPTimeoutPolicy const& getHTTPTimeoutPolicy(void) const { return baseFeaturesReceived_timeout; }
	char const* getName(void) const { return mClassName; }
private:
	void retry()
	{
...
void LLViewerRegionImpl::buildCapabilityNames(LLSD& capabilityNames)
{
	...
	capabilityNames.append("AcceptGroupInvite");
	...
	capabilityNames.append("DeclineGroupInvite");
	...
	capabilityNames.append("GamingData");
...
void LLViewerRegion::setCapability(const std::string& name, const std::string& url)
{
	...
	else
	{
		...
		if (name == "GetTexture" || name == "GetMesh" || name == "GetMesh2" || name == "ViewerAsset")
		{
			LL_INFOS("SceneLoad") << "cap " << name << " = " << url
								  << " region=" << getName() << LL_ENDL;
		}
		if(name == "ViewerAsset")
		{
