...
void camera_reset_on_motion()
{
	...
	else if (LLSelectMgr::getInstance()->getSelection()->isAttachment())
	{
	...
	if (mAutoPilot && isAgentAvatarValid())
	{
		...
		if (follow)
		{
			...
			if (auto object = gObjectList.findObject(mLeaderID))
			{
				...
				if (const auto& av = object->asAvatar())
				{
					const auto& our_pos_global = getPositionGlobal();
					setFlying(av->mInAir && (getFlying() || mAutoPilotTargetGlobal[VZ] > our_pos_global[VZ]));
...
void LLAgent::propagate(const F32 dt)
{
	...
	const F32 PITCH_RATE = 90.f * DEG_TO_RAD;
...
void LLAgent::endAnimationUpdateUI()
{
	...
	if (gAgentCamera.getLastCameraMode() == CAMERA_MODE_MOUSELOOK )
	{
		...
		if (gNavigationBar)
		{
			gNavigationBar->setVisibleForMouselook(true);
		}
		if (gNotifyIconBar)
		{
			gNotifyIconBar->setVisibleForMouselook(true);
		}
	...
	if (gAgentCamera.getCameraMode() == CAMERA_MODE_MOUSELOOK)
	{
		...
		if (gNavigationBar)
		{
			gNavigationBar->setVisibleForMouselook(false);
		}
		if (gNotifyIconBar)
		{
			gNotifyIconBar->setVisibleForMouselook(false);
		}
...
void LLAgent::sendRevokePermissions(const LLUUID & target, U32 permissions)
{
	if (gAgentID.notNull() && gMessageSystem)
	{
		...
		msg->addUUIDFast(_PREHASH_ObjectID, target);
...
bool LLAgent::teleportCore(bool is_local)
{
	...
	start_hud_teleport_logging();
	dump_hud_teleport_state("teleportCore_begin");
	...
	if (mTeleportRequest && mTeleportRequest->getStatus() == LLTeleportRequest::kFailed)
	{
		LL_WARNS("Teleport") << "Teleport request was marked failed before TELEPORT_START; continuing new request." << LL_ENDL;
	...
	else
	{
		...
		static const LLCachedControl<bool> skip_reset_objects_on_teleport("SHSkipResetVBOsOnTeleport", true);
		// Firestorm does not wipe vertex buffers on teleport. Doing so can
		// leave HUD attachments without geometry until a refresh rebuilds them.
		if(!hide_tp_screen && !skip_reset_objects_on_teleport)
		{
			LL_INFOS("HUDTeleport") << "teleportCore calling resetVertexBuffers" << LL_ENDL;
			gPipeline.resetVertexBuffers();
			LLSpatialPartition::sTeleportRequested = TRUE;
			dump_hud_teleport_state("teleportCore_after_resetVBOs");
		}
	...
	dump_hud_teleport_state("teleportCore_after_state");
	return true;
}
