...
void LLViewerTextureList::init()
{
	...
	updateMaxResidentTexMem(S32Megabytes(0));
	doPreloadImages();
}
												   ...
												   const LLUUID& force_id)
{
	...
	if (imagep.isNull())
	{
		...
		maybeAddToFastCache(imagep, boost_priority);
	}
...
void LLViewerTextureList::maybeAddToFastCache(LLViewerFetchedTexture* imagep, LLViewerTexture::EBoostLevel boost_priority)
{
	static LLCachedControl<bool> fast_cache_fetching_enabled(gSavedSettings, "FastCacheFetchEnabled", true);
	if (!fast_cache_fetching_enabled || !imagep)
	{
		return;
	}
	if (boost_priority == LLViewerFetchedTexture::BOOST_SCULPTED
		|| boost_priority == LLViewerFetchedTexture::BOOST_UI
		|| boost_priority == LLViewerFetchedTexture::BOOST_ICON
		|| imagep->getFTType() == FTT_LOCAL_FILE)
	{
		return;
	}
	const std::string& url = imagep->getUrl();
...
{
	if(gTeleportDisplay && gAgent.getTeleportState() != LLAgent::TELEPORT_ARRIVING)
	{
		return;
	}
	...
	}
	{
		...
		max_time = llmax(max_time, total_max_time*.50f);
...
{
	{
		F32 lazy_flush_timeout = 30.f;
		F32 max_inactive_time  = 20.f;
		S32 min_refs = 3;
		bool reset_timer = gViewerWindow->getProgressView()->getVisible();
        static const S32 MAX_PRIO_UPDATES = gSavedSettings.getS32("TextureFetchUpdatePriorities");
...
F32 LLViewerTextureList::updateImagesFetchTextures(F32 max_time)
{
	...
	static const S32 MAX_HIGH_PRIO_COUNT = gSavedSettings.getS32("TextureFetchUpdateHighPriority");
	static const S32 MAX_UPDATE_COUNT = gSavedSettings.getS32("TextureFetchUpdateMaxMediumPriority");
	static const S32 MIN_UPDATE_COUNT = gSavedSettings.getS32("TextureFetchUpdateMinMediumPriority");
	static const F32 MIN_PRIORITY_THRESHOLD = gSavedSettings.getF32("TextureFetchUpdatePriorityThreshold");
	static const bool SKIP_LOW_PRIO = gSavedSettings.getBOOL("TextureFetchUpdateSkipLowPriority");
...
void LLViewerTextureList::updateMaxResidentTexMem(S32Megabytes mem)
{
	...
	const S32Megabytes default_mem = getMaxVideoRamSetting(true, mem_multiplier);
	...
	S32Megabytes fb_mem = llmin(llmax(VIDEO_CARD_FRAMEBUFFER_MEM_MIN, mem / 4), VIDEO_CARD_FRAMEBUFFER_MEM_MAX);
	S32Megabytes misc_mem = llmax(MIN_MEM_FOR_NON_TEXTURE, mem / 5);
...
bool LLUIImageList::initFromFile()
{
	...
	for (S32 cur_pass = PASS_DECODE_NOW; cur_pass < NUM_PASSES; cur_pass++)
	{
		...
		{
			gTextureList.decodeAllImages(10.f);
		}
	}
	return true;
}
