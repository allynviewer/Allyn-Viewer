...
const F32 FIND_FREQUENCY = 29.7f;
...
static std::string friend_presence_label(const LLAvatarName& av_name)
{
	std::string nome = av_name.getLegacyName();
	if (nome.empty())
	{
		nome = av_name.getUserName();
	}
	const std::string display = av_name.getDisplayName();
	if (!display.empty() && display != nome)
	{
		return nome + " ( " + display + " )";
	}
	return nome;
...
{
	LLSD args;
	args["NAME"] = friend_presence_label(av_name);
	args["ID"] = agent_id;
	args["STATUS"] = online ? "\xF0\x9F\x9F\xA2" : "\xF0\x9F\x94\xB4";
