...
:	name("")
{
	...
	const U32 end = start + (mesh ? mesh->getNumVertices() : vb->getNumVerts()) - 1;
	...
	if (transform_normals) Transform(normals, transform_normals);
	const U32 pcount = mesh ? mesh->getNumFaces() : (vb->getNumIndices()/3);
	const U32 offset = face->getIndicesStart();
...
void Wavefront::Transform(vert_t& v, const LLXform* x)
{
	LLMatrix4 m;
	x->getLocalMat4(m);
	for (auto& i : v)
...
void Wavefront::Transform(vec3_t& v, const LLXform* x)
{
	LLMatrix4 m;
	x->getLocalMat4(m);
	for (auto& i : v)
...
void WavefrontSaver::Add(const LLVOAvatar* av_vo)
{
	...
	for (const auto& itervj : vjv)
	{
		...
		auto* vjm = dynamic_cast<LLViewerJointMesh*>(vj->mMeshParts[0]);
		if (!vjm) continue;
