...
void LLManipScale::render()
{
	...
	if( canAffectSelection() )
	{
		...
		const F32 BOX_HANDLE_BASE_SIZE		= 50.0f;
		...
		else
		{
			for (S32 i = 0; i < NUM_MANIPULATORS; i++)
			{
				...
				if (range_squared > 0.001f * 0.001f)
				{
					...
					F32 apparent_angle = fraction_of_fov * LLViewerCamera::getInstance()->getView();
...
void LLManipScale::dragFace( S32 x, S32 y )
{
	...
	{
		mSnapRegime = static_cast<ESnapRegimes>(SNAP_REGIME_UPPER | SNAP_REGIME_LOWER);
...
void LLManipScale::stretchFace( const LLVector3& drag_start_agent, const LLVector3& drag_delta_agent )
{
		 ...
		 iter != mObjectSelection->end(); iter++)
	{
			...
			!cur->isAvatar() )
		{
			...
			F32 desired_delta_size	= is_approx_zero(denom) ? 0.f : (delta_local_mag / denom);
