...
const F32 AUTOPLAY_TIME  = 5;
const F32 AUTOPLAY_SIZE  = 24*24;
const F32 AUTOPLAY_SPEED = 0.1f;
...
BOOL LLViewerParcelMediaAutoPlay::tick()
{
	...
	if ((!mPlayed) &&
		(mTimeInParcel > AUTOPLAY_TIME) &&
		(!this_media_url.empty()) &&
		(stricmp(this_media_type.c_str(), LLMIMETypes::getDefaultMimeType().c_str()) != 0) &&
		(LLViewerParcelMedia::sMediaImpl.isNull()))
	{
		if (this_media_texture_id.notNull())
		{
			...
			if (gAgent.getVelocity().magVec() < AUTOPLAY_SPEED)
			{
