...
{
	U32 flags = PERM_NONE;
	if (prefix != "Bulk" && LFSimFeatureHandler::instance().simSupportsExport() && prefix.empty() && gSavedPerAccountSettings.getBOOL(prefix+"EveryoneExport"))
		flags |= PERM_EXPORT;
...
BOOL LLFloaterPermsDefault::postBuild()
{
	...
	if (is_sl)
	{
		...
		S32 shift(view->getRect().getWidth());
		LLRect rect(getRect());
		rect.mRight -= shift;
		setRect(rect);
