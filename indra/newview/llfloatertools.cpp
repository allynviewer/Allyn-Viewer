	...
	mNeedMediaTitle(TRUE)
{
	...
	factory_map["ContentsInventory"] = LLCallbackMap(createPanelContentsInventory, this);
...
void LLFloaterTools::refresh()
{
...
#endif
	{
		...
		{
			if (node->getTESelectMask() == 0xFFFFFFFF)
			{
				selected_face = -2;
			}
			if (const auto& obj = node->getObject())
			{
				...
				for (S32 te = 0; selected_face != -2 && te < numTEs; ++te)
				{
		...
		if (mRadioSelectFace->getValue() || selected_face > 0)
		{
		...
		else if (edit_linked && prim_count == 1)
		{
...
void LLFloaterTools::updatePopup(LLCoordGL center, MASK mask)
{
	...
	if (mCheckSnapToGrid) mCheckSnapToGrid->setVisible( edit_visible );
	if (mBtnGridOptions) mBtnGridOptions->setVisible( edit_visible );
	...
	getChildView("focus_panel")->setVisible(focus_visible);
	getChildView("move_panel")->setVisible(move_visible);
	getChildView("edit_panel")->setVisible(edit_visible);
	getChildView("create_panel")->setVisible(create_visible);
	getChildView("land_panel")->setVisible(land_visible);
...
void commit_slider_zoom(LLUICtrl *ctrl)
{
	F32 zoom_level = (F32)ctrl->getValue().asReal() * 2.f;
