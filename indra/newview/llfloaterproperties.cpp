...
void LLFloaterProperties::refreshFromItem(LLInventoryItem* item)
{
	...
	getChildView("LabelItemName")->setEnabled(is_modifiable && !is_calling_card);
	...
	if( gSavedSettings.getBOOL("DebugPermissions") )
	{
		...
		if (!supports_export && base_mask & PERM_EXPORT)
		...
		if (!supports_export && owner_mask & PERM_EXPORT)
			perm_string.erase(perm_string.find_last_of("E"));
		...
		if (!supports_export && everyone_mask & PERM_EXPORT)
			perm_string.erase(perm_string.find_last_of("E"));
		...
		&& gAgent.allowOperation(PERM_TRANSFER, perm, GP_OBJECT_MANIPULATE))
	{
		...
		bool no_export = !(everyone_mask & PERM_EXPORT);
