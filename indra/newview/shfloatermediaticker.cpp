...
SHFloaterMediaTicker::SHFloaterMediaTicker() : LLFloater(),
...
BOOL SHFloaterMediaTicker::postBuild()
{
...
void SHFloaterMediaTicker::draw()
{
	updateTickerText();
	LLFloater::draw();
	drawOscilloscope();
}
void SHFloaterMediaTicker::onOpen()
{
	LLFloater::onOpen();
	gSavedSettings.setBOOL("SHShowMediaTicker", TRUE);
}
void SHFloaterMediaTicker::onClose(bool app_quitting)
{
...
void SHFloaterMediaTicker::reshape(S32 width, S32 height, BOOL called_from_parent)
{
...
void SHFloaterMediaTicker::updateTickerText()
{
	if(!gAudiop)
		return;
	bool stream_paused = gAudiop->getStreamingAudioImpl()->isPlaying() != 1;
	...
	if(!stream_paused)
	{
		if(dirty || mUpdateTimer.getElapsedTimeF64() >= .5f)
		{
			...
			if(artist.isDefined() && title.isDefined())
				...
				mLoadTimer.start();
			else if(mLoadTimer.getStarted() && mLoadTimer.getElapsedTimeF64() > 10.f)
			{
				if(!artist.isDefined())
					dirty |= setArtist("");
				if(!title.isDefined())
...
void SHFloaterMediaTicker::drawOscilloscope()
{
	if(!gAudiop || !mVisualizer || !gAudiop->getStreamingAudioImpl()->supportsWaveData())
		return;
	static const S32 NUM_LINE_STRIPS = 64;
	static const S32 WAVE_DATA_STEP_SIZE = 4;
	static const S32 NUM_WAVE_DATA_VALUES = NUM_LINE_STRIPS*WAVE_DATA_STEP_SIZE;
	static F32 buf[NUM_WAVE_DATA_VALUES];
	LLRect root_rect = mVisualizer->getRect();
	F32 height = root_rect.getHeight();
	F32 height_scale = height / 2.f;
...
void SHFloaterMediaTicker::iterateTickerOffset()
{
		 ...
		 ( mCurScrollChar && mScrollTimer.getElapsedTimeF32() >= .5f)))
	{
		...
		{
			if(mScrollTimer.getElapsedTimeF32() >= 2.f)
				resetTicker();
		}
		else
		{
