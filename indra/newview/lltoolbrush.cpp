									...
									const LLVector3& pos_world)
{
	...
	S32 radioAction = gSavedSettings.getS32("RadioLandBrushAction");
	F32 force = gSavedSettings.getF32("LandBrushForce");
	...
	for(S32 di = -half_edge; di <= half_edge; di++)
	{
		...
		for(S32 dj = -half_edge; dj <= half_edge; dj++)
		{
			...
			const F32
				...
				force_scale = sqrt(2.f) - norm_dist,
				wz2 = wz + .2 + (.2 + force/100) * force_scale,
				tic = .075f;
			gGL.vertex3f(wx, wy, wz);
			gGL.vertex3f(wx, wy, wz2);
			if(radioAction == E_LAND_RAISE || radioAction == E_LAND_NOISE)
			{
				gGL.vertex3f(wx, wy, wz2);
			...
			if(radioAction == E_LAND_LOWER || radioAction == E_LAND_NOISE)
			{
				gGL.vertex3f(wx, wy, wz);
			...
			if(radioAction == E_LAND_REVERT || radioAction == E_LAND_SMOOTH)
			{
				gGL.vertex3f(wx-tic, wy, wz2);
				gGL.vertex3f(wx+tic, wy, wz2);
			}
			if(radioAction == E_LAND_LEVEL || radioAction == E_LAND_SMOOTH)
			{
