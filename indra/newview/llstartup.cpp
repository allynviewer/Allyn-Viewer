...
static std::string sInitialOutfitGender;
...
{
	LLAppViewer::getTextureCache()->update(1);
	LLAppViewer::getImageDecodeThread()->update(1);
	LLAppViewer::getTextureFetch()->update(1);
...
bool idle_startup()
{
	...
	if ( STATE_FIRST == LLStartUp::getStartupState() )
	{
		...
		found_template = LLFile::fopen(message_template_path, "r");
		if (!found_template)
		{
	...
	if (STATE_LOGIN_SHOW == LLStartUp::getStartupState())
	{
		...
		if (show_connect_box)
		{
			...
			login_show();
			display_startup();
			static bool sSetFields(LLPanelLogin::hasLoginHistory());
			if (!sSetFields)
			{
		...
		window.show(false);
		if (gSavedSettings.getBOOL("LiruFlashWhenMinimized"))
	...
	if (STATE_LOGIN_CLEANUP == LLStartUp::getStartupState())
	{
		...
		LLScriptEdCore::parseFunctions("lsl_functions_sl.xml");
		gHippoGridManager->setCurrentGridAsConnected();
		gHippoLimits->setLimits();
		if(!gHippoGridManager->getConnectedGrid()->isSecondLife())
		{
			LLTrans::setDefaultArg("[CURRENCY]",gHippoGridManager->getConnectedGrid()->getCurrencySymbol());
			LLTrans::setDefaultArg("[CURRENCY_TEXT]",gHippoGridManager->getConnectedGrid()->getCurrencyText());
	...
	if(STATE_LOGIN_PROCESS_RESPONSE == LLStartUp::getStartupState())
	{
		...
		case LLUserAuth::E_OK:
			...
			if(login_response == "true")
			{
				...
				Debug(if (gCurlIo) dc::curlio.off());
				LLPanelLogin::close();
			}
	...
	if (STATE_SEED_CAP_GRANTED == LLStartUp::getStartupState())
	{
		...
		display_startup();
		gMessageSystem->setMaxMessageTime( 0.5f );
		display_startup();
		#ifndef	LL_RELEASE_FOR_DOWNLOAD
			gMessageSystem->setTimeDecodes( TRUE );
			gMessageSystem->setTimeDecodesSpamThreshold( 0.05f );
		#endif
		display_startup();
		gXferManager->registerCallbacks(gMessageSystem);
		display_startup();
		LLStartUp::initNameCache();
		LLLogChat::initializeIDMap();
		display_startup();
	...
	if (STATE_PRECACHE == LLStartUp::getStartupState())
	{
		...
		if (gAgent.isFirstLogin()
			&& !sInitialOutfit.empty()
			&& !sInitialOutfitGender.empty()
			&& isAgentAvatarValid()
...
std::string LLStartUp::loadPasswordFromDisk()
{
	...
	std::string filepath = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS,
													   "password.dat");
	LLFILE* fp = LLFile::fopen(filepath, "rb");
	if (!fp)
	{
		return hashed_password;
	}
...
void LLStartUp::savePasswordToDisk(const std::string& hashed_password)
{
	std::string filepath = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS,
													   "password.dat");
	LLFILE* fp = LLFile::fopen(filepath, "wb");
	if (!fp)
	{
		return;
	}
...
bool process_login_success_response(std::string& password, U32& first_sim_size_x, U32& first_sim_size_y)
{
	...
	else if (opensim) tmp = grid.getSearchUrl();
	if (opensim)
	{
		gSavedSettings.setString("SearchURL", tmp);
