...
void LLFloaterModelPreview::loadModel_continued(AIFilePicker* filepicker, S32 lod)
{
	...
	mModelPreview->loadModel(filename, lod);
}
...
void LLModelPreview::updateLodControls(S32 lod)
{
	...
	else if (lod_mode == USE_LOD_ABOVE)
	{
		fmp->mLODMode[lod] = 2;
		for (U32 i = 0; i < num_file_controls; ++i)
		{
			...
			if (skinned)
			{
				for (U32 i = 0; i < num_vertices; i++)
				{
					...
					for (U32 i = 0; i < weight_list.size(); ++i)
					{
						...
						w.mV[i] = joint + wght;
						llassert(w.mV[i]-(S32)w.mV[i]>0.0f);
					}
...
BOOL LLModelPreview::render()
{
	...
	LLViewerCamera::getInstance()->setOriginAndLookAt(
													  target_pos + ((LLVector3(mCameraDistance, 0.f, 0.f) + offset) * av_rot),
													  LLVector3::z_axis,
													  target_pos);
...
void LLModelPreview::setPreviewLOD(S32 lod)
{
	...
	if (lod != mPreviewLOD)
	{
		...
		combo_box2->setCurrentByIndex((NUM_LOD-1)-mPreviewLOD);
		LLComboBox* combo_box3 = mFMP->getChild<LLComboBox>("preview_lod_combo3");
		combo_box3->setCurrentByIndex((NUM_LOD-1)-mPreviewLOD);
