...
void LLPanelGroups::filterGroups(const std::string& search_name)
{
	...
	{
		if (search_name.find(mLastGroupSearch) == std::string::npos)
		{
			init_group_list(group_list, gAgent.getGroupID());
		}
		std::vector<LLScrollListItem*> groups = group_list->getAllData();
		for (std::vector<LLScrollListItem*>::iterator itr = groups.begin(); itr != groups.end(); ++itr)
		{
			std::string group_name = utf8str_tolower((*itr)->getColumn(0)->getValue().asString());
			if (group_name.find(utf8str_tolower(search_name)) == std::string::npos)
			{
				group_list->deleteItems((*itr)->getValue());
			}
		}
		group_list->updateLayout();
		enableButtons();
	}
	else if (!mLastGroupSearch.empty())
	{
		init_group_list(group_list, gAgent.getGroupID());
		enableButtons();
	}
	mLastGroupSearch = search_name;
}
void LLPanelGroups::reset()
{
	setTexts();
	init_group_list(getChild<LLScrollListCtrl>("group list"), gAgent.getGroupID());
	if (!mLastGroupSearch.empty())
	{
		std::string saved = mLastGroupSearch;
		mLastGroupSearch.clear();
		filterGroups(saved);
	}
	enableButtons();
}
BOOL LLPanelGroups::postBuild()
{
	...
	if (list)
	{
		...
		list->setSortChangedCallback(boost::bind(&LLPanelGroups::onGroupSortChanged,this));
		list->setDoubleClickCallback(boost::bind(LLGroupActions::startIM, boost::bind(&LLScrollListCtrl::getCurrentID, list)));
	}
	if (LLFilterEditor* group_search = getChild<LLFilterEditor>("group_search_lineedit"))
	{
		group_search->setCommitCallback(boost::bind(&LLPanelGroups::filterGroups, this, _2));
	}
...
void init_group_list(LLScrollListCtrl* ctrl, const LLUUID& highlight_id, U64 powers_mask)
{
	...
	if(selected_idx!=group_list->getFirstSelectedIndex())
