...
bool LLInventoryFilter::checkAgainstFilterType(const LLFolderViewItem* item) const
{
	...
	if (filterTypes & FILTERTYPE_WORN)
	{
		...
		return (get_is_item_worn(object_id)
				&& !LLAppearanceMgr::instance().getIsInCOF(object_id)
				&& (!cat || cat->getPreferredType() != LLFolderType::FT_OUTFIT)
				&& !object->getIsLinkType());
	}
	return TRUE;
}
...
bool LLInventoryFilter::FilterOps::DateRange::validateBlock( bool   emit_errors ) const
{
