...
const S32Megabytes gMaxVideoRam(2048*4);
...
S32 LLViewerTexture::sMaxSculptRez = 128;
const S32 MAX_CACHED_RAW_IMAGE_AREA = 64 * 64;
const S32 MAX_CACHED_RAW_SCULPT_IMAGE_AREA = LLViewerTexture::sMaxSculptRez * LLViewerTexture::sMaxSculptRez;
const S32 MAX_CACHED_RAW_TERRAIN_IMAGE_AREA = 128 * 128;
const S32 DEFAULT_ICON_DIMENTIONS = 32;
S32 LLViewerTexture::sMinLargeImageSize = 65536;
S32 LLViewerTexture::sMaxSmallImageSize = MAX_CACHED_RAW_IMAGE_AREA;
BOOL LLViewerTexture::sFreezeImageScalingDown = FALSE;
F32 LLViewerTexture::sCurrentTime = 0.0f;
F32  LLViewerTexture::sTexelPixelRatio = 1.0f;
LLViewerTexture::EDebugTexels LLViewerTexture::sDebugTexelsMode = LLViewerTexture::DEBUG_TEXELS_OFF;
const F32 desired_discard_bias_min = -2.0f;
const F32 desired_discard_bias_max = (F32)MAX_DISCARD_LEVEL;
...
{
	LLPointer<LLGLTexture> getLocalTexture(BOOL usemipmaps = TRUE, BOOL generate_gl_tex = TRUE)
	{
		return LLViewerTextureManager::getLocalTexture(usemipmaps, generate_gl_tex);
	}
	LLPointer<LLGLTexture> getLocalTexture(const U32 width, const U32 height, const U8 components, BOOL usemipmaps, BOOL generate_gl_tex = TRUE)
	{
		return LLViewerTextureManager::getLocalTexture(width, height, components, usemipmaps, generate_gl_tex);
	}
	LLGLTexture* getFetchedTexture(const LLUUID &image_id)
	{
...
const S32 min_non_tex_system_mem = (128<<20);
F32 texmem_lower_bound_scale = 0.85f;
F32 texmem_middle_bound_scale = 0.925f;
static LLTrace::BlockTimerStatHandle FTM_TEXTURE_MEMORY_CHECK("Memory Check");
bool LLViewerTexture::isMemoryForTextureLow()
{
	const F32 WAIT_TIME = 1.0f;
	static LLFrameTimer timer;
	if(timer.getElapsedTimeF32() < WAIT_TIME)
	{
		return false;
	}
	timer.reset();
...
void LLViewerTexture::updateClass(const F32 velocity, const F32 angular_velocity)
{
	...
	sMaxDesiredTextureMem = sMaxTotalTextureMem;
...
{
	static const F32 MAX_WAIT_TIME = 20.f;
...
{
	static const S32 MAX_INTERVAL = 8;
	mForSculpt = TRUE;
...
void LLViewerFetchedTexture::loadFromFastCache()
{
	...
	mInFastCacheList = false;
	if (mRawImage.notNull() || mFTType == FTT_LOCAL_FILE)
	{
		return;
	}
	S32 discard = mRawDiscardLevel;
	LLPointer<LLImageRaw> cached = LLAppViewer::getTextureCache()->readFromFastCache(getID(), discard);
	if (cached.isNull())
	{
		return;
	}
	mRawImage = cached;
	mRawDiscardLevel = discard;
...
const F32 MAX_PRIORITY_PIXEL                         = 999.f;
const F32 PRIORITY_BOOST_LEVEL_FACTOR                = 1000.f;
const F32 PRIORITY_DELTA_DISCARD_LEVEL_FACTOR        = 100000.f;
const S32 MAX_DELTA_DISCARD_LEVEL_FOR_PRIORITY       = 4;
const F32 PRIORITY_ADDITIONAL_FACTOR                 = 1000000.f;
const S32 MAX_ADDITIONAL_LEVEL_FOR_PRIORITY          = 8;
const F32 PRIORITY_BOOST_HIGH_FACTOR                 = 10000000.f;
F32 LLViewerFetchedTexture::calcDecodePriority()
{
#ifndef LL_RELEASE_FOR_DOWNLOAD
	if (mID == LLAppViewer::getTextureFetch()->mDebugID)
	{
		LLAppViewer::getTextureFetch()->mDebugCount++;
	}
#endif
	if (mNeedsCreateTexture)
	{
...
{
	static const F32 max_priority = PRIORITY_BOOST_HIGH_FACTOR +
		PRIORITY_ADDITIONAL_FACTOR * (MAX_ADDITIONAL_LEVEL_FOR_PRIORITY + 1) +
		PRIORITY_DELTA_DISCARD_LEVEL_FACTOR * (MAX_DELTA_DISCARD_LEVEL_FOR_PRIORITY + 1) +
		PRIORITY_BOOST_LEVEL_FACTOR * (BOOST_MAX_LEVEL - 1) +
		MAX_PRIORITY_PIXEL + 1.0f;
	return max_priority;
}
...
bool LLViewerFetchedTexture::isActiveFetching()
{
	...
	return mFetchState > 7 && mFetchState < 10 && monitor_enabled;
}
bool LLViewerFetchedTexture::updateFetch()
{
	...
	if(textures_decode_disabled ||
	   (gUseWireframe && mBoostLevel < LLGLTexture::BOOST_AVATAR_BAKED_SELF))
	{
		return false;
	}
	mFetchState = 0;
	...
	if (mIsFetching)
	{
		...
		{
			static const F32 MAX_HOLD_TIME = 5.0f;
	...
	else if(mCachedRawImage.notNull()
...
{
	static const F32 MAX_INACTIVE_TIME = 900.f ;
	static const F32 MAX_IDLE_WAIT_TIME = 5.f ;
	if (mNeedsCreateTexture)
	{
		return false;
	}
...
void LLViewerLODTexture::processTextureStats()
{
	...
	else
	{
		...
		if (mBoostLevel < LLGLTexture::BOOST_SCULPTED)
		{
			...
			discard_level *= sDesiredDiscardScale;
...
void LLViewerMediaTexture::addFace(U32 ch, LLFace* facep)
{
	...
	{
		mTextureList.push_back(facep->getTexture());
		return;
	}
	if(te && te->getID().notNull())
	{
...
LLMetricPerformanceTesterWithSession::LLTestSession* LLTexturePipelineTester::loadTestSession(LLSD* log)
{
	...
	while (in_log)
	{
		...
		if(start_time - start_fetching_time > F_ALMOST_ZERO)
		{
