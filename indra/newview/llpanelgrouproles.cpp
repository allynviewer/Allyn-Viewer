...
void LLPanelGroupMembersSubTab::applyMemberChanges()
{
		 ...
		 member != mMemberRoleChangeData.end(); ++member)
	{
		...
		{
			gdatap->changeRoleMember(role->first,
									 member->first,
									 role->second);
		}
		member->second->clear();
...
void LLPanelGroupMembersSubTab::addMemberToList(LLGroupMemberData* data)
{
	...
	item_params.columns.add().column("name").font("SANSSERIF_SMALL").font_style("NORMAL");
	item_params.columns.add().column("donated").value(donated.getString())
			.font("SANSSERIF_SMALL").font_style("NORMAL");
...
void LLPanelGroupBanListSubTab::handleBanEntrySelect()
{
	if (gAgent.hasPowerInGroup(mGroupID, GP_GROUP_BAN_ACCESS))
	{
		mDeleteBanButton->setEnabled(!!mBanList->getFirstSelected());
	}
}
...
void LLPanelGroupBanListSubTab::populateBanList()
{
	...
	for(; entry != gdatap->mBanList.end(); entry++)
	{
		...
		ban_entry.columns.add().column("name").font("SANSSERIF_SMALL").font_style("NORMAL");
		ban_entry.columns.add().column("ban_date").value(bd.mBanDate).type("date").format("%Y/%m/%d").font("SANSSERIF_SMALL").font_style("NORMAL");
