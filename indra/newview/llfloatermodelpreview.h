...
public:
	...
	bool isModelLoading();
	void onPermissionsReceived(const LLSD& result);
	void setPermissonsErrorStatus(U32 status, const std::string& reason);
	void onModelPhysicsFeeReceived(const LLSD& result, std::string upload_url);
				void handleModelPhysicsFeeReceived();
	void setModelPhysicsFeeErrorStatus(U32 status, const std::string& reason);
	void onModelUploadSuccess();
	void onModelUploadFailure();
