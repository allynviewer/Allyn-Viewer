...
public:
	...
	U32 drawShape( F32 pixelArea, BOOL first_pass, BOOL is_dummy );
	U32 render( F32 pixelArea, BOOL first_pass = TRUE, BOOL is_dummy = FALSE ) { return LLViewerJoint::render(pixelArea,first_pass,is_dummy);}
	void updateFaceSizes(U32 &num_vertices, U32& num_indices, F32 pixel_area);
	void updateFaceData(LLFace *face, F32 pixel_area, BOOL damp_wind = FALSE, bool terse_update = false);
	BOOL updateLOD(F32 pixel_area, BOOL activate);
	void updateJointGeometry();
	void dump();
