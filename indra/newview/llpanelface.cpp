...
const S32 MATMEDIA_MATERIAL = 0;
const S32 MATMEDIA_MEDIA = 1;
const S32 MATTYPE_DIFFUSE = 0;
const S32 MATTYPE_NORMAL = 1;
const S32 MATTYPE_SPECULAR = 2;
const S32 ALPHAMODE_NONE = 0;
const S32 ALPHAMODE_BLEND = 1;
const S32 ALPHAMODE_MASK = 2;
const S32 BUMPY_TEXTURE = 18;
const S32 SHINY_TEXTURE = 4;
...
void LLPanelFace::updateUI()
{
		...
		&& objectp->permModify())
	{
		...
		texture_ctrl->setFallbackImageName( "" );
		LLTextureCtrl*	shinytexture_ctrl = mShinyTextureCtrl;
		shinytexture_ctrl->setFallbackImageName( "" );
		LLTextureCtrl*	bumpytexture_ctrl = mBumpyTextureCtrl;
		bumpytexture_ctrl->setFallbackImageName( "" );
		LLUUID id;
		LLUUID normmap_id;
		LLUUID specmap_id;
		{
...
void LLPanelFace::updateShinyControls(bool is_setting_texture, bool mess_with_shiny_combobox)
{
	...
	bool show_shinyctrls = (shiny_value == SHINY_TEXTURE) && show_shininess;
...
void LLPanelFace::updateAlphaControls()
{
	...
	bool show_alphactrls = (alpha_value == ALPHAMODE_MASK);
	LLComboBox* combobox_matmedia = mComboMatMedia;
...
void LLPanelFace::onTextureSelectionChanged(LLInventoryItem* itemp)
{
	...
	if (texture_ctrl)
	{
		...
		bool can_copy = itemp->getPermissions().allowCopyBy(gAgentID);
		bool can_transfer = itemp->getPermissions().allowOperationBy(PERM_TRANSFER, gAgentID);
		bool is_object_owner = gAgentID == obj_owner_id;
		bool not_for_sale = !sale_info.isForSale();
