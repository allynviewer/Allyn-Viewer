...
BOOL LLFace::sSafeRenderSelect = TRUE;
...
bool LLFace::canRenderAsMask()
{
	...
	if ((te->getColor().mV[3] == 1.0f) &&
		(te->getGlow() == 0.f) &&
		!getViewerObject()->isHUDAttachment() &&
		(getTexture()->getIsAlphaMask((!getViewerObject()->isAttachment() && use_rmse_auto_mask) ? auto_mask_max_rmse : -1.f, auto_mask_max_mid)))
	{
...
{
	if (source_count > 0 && dest_count >= source_count)
	{
...
BOOL LLFace::hasMedia() const
{
	...
	{
		return mTexture[LLRender::DIFFUSE_MAP]->hasParcelMedia() ;
	}
	return FALSE ;
}
...
F32 LLFace::getTextureVirtualSize()
{
	...
	if(face_area > LLViewerTexture::sMinLargeImageSize)
	{
...
const F32 FACE_IMPORTANCE_TO_CAMERA_OVER_DISTANCE[FACE_IMPORTANCE_LEVEL][2] =
	{{16.1f, 1.0f}, {32.1f, 0.5f}, {48.1f, 0.2f}, {96.1f, 0.05f} } ;
const F32 FACE_IMPORTANCE_TO_CAMERA_OVER_ANGLE[FACE_IMPORTANCE_LEVEL][2] =
	{{0.985f , 1.0f}, {0.94f , 0.8f}, {0.866f , 0.64f}, {0.0f, 0.36f}} ;
