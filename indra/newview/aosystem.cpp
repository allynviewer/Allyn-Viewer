...
public:
	static bool needed()
	{
		return LLStartUp::getStartupState() < STATE_INVENTORY_SEND
...
void AOSystem::start()
{
	llassert(!instanceExists() && !AOInvTimer::instanceExists());
	auto control(gSavedSettings.getControl("AOEnabled"));
	if (control->get()) AOInvTimer::createIfNeeded();
	control->getSignal()->connect([](LLControlVariable*, const LLSD& enabled) {
		if (enabled.asBoolean()) AOInvTimer::createIfNeeded();
...
{
	uuid_vec_t anims;
	if (!start || gSavedSettings.getBOOL("PlayTypingAnim"))
		anims.push_back(ANIM_AGENT_TYPE);
	if (const auto& ao = getIfExists())
...
void AOSystem::updateStand()
{
	...
	if (is_standing) stand_ao.play(false);
	stand_ao.update(mAOStands, stand_iterator);
	if (is_standing && !mAOStands.empty())
		stand_ao.play();
}
					...
					if (boost::regex_search(strline, what, type))
					{
						...
						for (const auto& stranim : tokanimnames)
						{
							...
							if (animid.notNull())
							{
								...
								if (combo && !combo->selectByValue(stranim))
