...
bool LLMemory::isMemoryPoolLow()
{
	...
	if(sAvailPhysicalMemInKB < (LOW_MEMORY_POOL_THRESHOLD / 4))
	{
		return true ;
	}
	if(sAllocatedPageSizeInKB + (LOW_MEMORY_POOL_THRESHOLD / 4) > sMaxHeapSizeInKB)
	{
		return true ;
	}
...
{
	static const S32 MIN_ALLOCATION = 0 ;
	if(mPaused)
	{
		return ;
	}
...
void LLPrivateMemoryPool::LLMemoryBlock::init(char* buffer, U32 buffer_size, U32 slot_size)
{
	...
	llassert_always(buffer_size / mSlotSize <= MAX_NUM_SLOTS_IN_A_BLOCK) ;
	mAllocatedSlots = 0 ;
	mDummySize = 0 ;
	if(mTotalSlots > 32)
	{
...
{
	if(data_buffer_size / min_slot_size < 64)
	{
...
U32 LLPrivateMemoryPool::LLMemoryChunk::calcBlockSize(U32 slot_size)
{
	...
	if((slot_size << 5) <= mMinBlockSize)
	{
		block_size = mMinBlockSize ;
	}
	else if(slot_size >= cut_off_size)
	{
...
LLPrivateMemoryPool::LLMemoryBlock* LLPrivateMemoryPool::LLMemoryChunk::addBlock(U32 blk_idx)
{
	...
	else
	{
		...
		for(S32 i = (S32)idx - 1 ; i >= min_idx ; i--)
		{
			if(mFreeSpaceList[i])
			{
				U32 new_preferred_block_size = mFreeSpaceList[i]->getBufferSize();
				new_preferred_block_size = (new_preferred_block_size / mMinBlockSize) * mMinBlockSize ;
				if(new_preferred_block_size >= slot_size)
				{
		...
		if(!blk)
		{
			for(U16 i = idx + 1 ; i < mPartitionLevels - 1; i++)
			{
				if(mFreeSpaceList[i])
				{
...
LLPrivateMemoryPool::LLMemoryBlock* LLPrivateMemoryPool::LLMemoryChunk::createNewBlock(LLMemoryBlock* blk, U32 buffer_size, U32 slot_size, U32 blk_idx)
{
	...
	if(new_free_blk_size < mMinBlockSize)
	{
		new_free_blk_size = 0 ;
	}
	if(new_free_blk_size > 0)
	{
...
void LLPrivateMemoryPool::LLMemoryChunk::removeBlock(LLMemoryBlock* blk)
{
	...
	if(blk->getBuffer() + blk->getBufferSize() <= mBuffer + mBufferSize - mMinBlockSize)
	{
...
const U32 CHUNK_SIZE = 4 << 20 ;
const U32 LARGE_CHUNK_SIZE = 4 * CHUNK_SIZE ;
...
LLPrivateMemoryPool::LLMemoryChunk* LLPrivateMemoryPool::addChunk(S32 chunk_index)
{
	...
	{
		preferred_size = LARGE_CHUNK_SIZE ;
		overhead = LLMemoryChunk::getMaxOverhead(preferred_size, MIN_SLOT_SIZES[chunk_index],
			MAX_SLOT_SIZES[chunk_index], MIN_BLOCK_SIZES[chunk_index], MAX_BLOCK_SIZES[chunk_index]) ;
	}
...
void LLPrivateMemoryPool::addToHashTable(LLMemoryChunk* chunk)
{
	...
	if(need_rehash)
	{
		...
		llassert_always(mHashFactor != 0xFFFF) ;
		rehash() ;
	}
}
...
LLPrivateMemoryPoolManager::LLPrivateMemoryPoolManager(BOOL enabled, U32 max_pool_size)
{
	...
	const U32 MAX_POOL_SIZE = 256 * 1024 * 1024 ;
