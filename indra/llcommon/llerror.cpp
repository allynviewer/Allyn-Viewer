...
namespace {
	...
	public:
					   ...
					   const std::string& message)
		{
			...
			fflush(stderr);
#endif
			if (ANSI_YES == mUseANSI) colorANSI("0");
		}
	private:
		enum ANSIState
		{
...
namespace
{
	...
	private:
		...
		friend class AIThreadSafeSimpleDC<Globals>;
		friend class AIThreadSafeSimple<Globals>;
		Globals();
	};
	Globals::Globals()
		: messageStream(),
...
namespace LLError
{
	...
	private:
		friend class AIThreadSafeBits<Settings>;
		friend class AIThreadSafeSimpleDC<Settings>;
		Settings();
