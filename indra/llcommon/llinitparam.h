...
namespace LLInitParam
{
	...
	public:
		...
		param_map_t						mNamedParams;
		param_list_t					mUnnamedParams;
		param_validation_list_t			mValidationList;
		all_params_list_t				mAllParams;
		size_t							mMaxParamOffset;
		EInitializationState			mInitializationState;
		class BaseBlock*				mCurrentBlockPtr;
	};
		template<typename T>
	class LazyValue
		{
	...
	public:
		...
		static bool deserializeParam(Param& param, Parser& parser, Parser::name_stack_range_t& name_stack_range, bool new_name)
		{
			...
			if (new_name_stack_range.first == new_name_stack_range.second)
			{
				...
				else if (parser.readValue(value))
				{
					typed_param.add(value);
					return true;
				}
	...
	public:
		...
		{
			if ((overwrite && source_provided)
				|| (!overwrite && !dst_provided))
			{
				resetToDefault();
