...
protected:
	static LLVFS*				sVFS;
	JointMotionListPtr				mJointMotionList;
...
private:
	...
	~LLKeyframeDataCache();
public:
	typedef AICachedPointer<LLUUID, LLKeyframeMotion::JointMotionList>::container_type keyframe_data_map_t;
	static keyframe_data_map_t sKeyframeDataMap;
	static LLKeyframeMotion::JointMotionListPtr createKeyframeData(LLUUID const& id);
	static LLKeyframeMotion::JointMotionListPtr    getKeyframeData(LLUUID const& id);
	static void removeKeyframeData(const LLUUID& id);
	static void dumpDiagInfo(int quiet = 0);
	static void clear();
};
#endif
