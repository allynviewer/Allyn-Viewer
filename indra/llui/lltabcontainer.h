...
public:
	...
	LLView* getChildView(const std::string& name, BOOL recurse = TRUE, BOOL create_if_missing = TRUE) const;
	...
	void		setTabUnreadCount(LLPanel* child, S32 count);
	...
	void		setTabVisibility(LLPanel const* panel, BOOL visible);
	BOOL		getTabVisibility(const LLPanel* panel) const;
	S32			getVisibleTabCount() const;
	void		setUserResize(BOOL enable) { mUserResize = enable; }
	BOOL		getUserResize() const { return mUserResize; }
	void		setVerticalTabWidth(S32 width);
	S32			getVerticalTabWidth() const { return mMinTabWidth; }
	void		setTabWidthCallback(const boost::function<void(S32)>& cb) { mTabWidthCallback = cb; }
...
private:
	...
	void scrollPrev() { mScrollPos = llmax(0, mScrollPos-1); }
	void scrollNext() { mScrollPos = llmin(mScrollPos+1, mMaxScrollPos); }
	void updateMaxScrollPos();
	void commitHoveredButton(S32 x, S32 y);
	void reshapeTuple(LLTabTuple* tuple);
	bool		isOverVerticalSplitter(S32 x, S32 y) const;
	S32			getVerticalSplitterX() const;
	S32			clampVerticalTabWidth(S32 width) const;
	...
	BOOL							mHoverSplitter;
	S32								mDragStartX;
	S32								mDragStartTabWidth;
	boost::function<void(S32)>		mTabWidthCallback;
