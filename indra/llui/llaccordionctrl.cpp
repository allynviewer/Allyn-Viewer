...
void	LLAccordionCtrl::initNoTabsWidget(const std::string& tb_params)
{
	mNoMatchedTabsOrigString = tb_params;
	mNoVisibleTabsHelpText = new LLTextBox(tb_params, getLocalRect());
}
...
{
	S32 panel_left = BORDER_MARGIN;
	S32 panel_top = getRect().getHeight() - BORDER_MARGIN;
	...
	for(size_t i=0;i<mAccordionTabs.size();++i)
	{
		...
		if(accordion_tab->getVisible() == false)
			continue;
...
S32 LLAccordionCtrl::calcExpandedTabHeight(S32 tab_index , S32 available_height )
{
	if(tab_index < 0)
	{
	...
	S32 expanded_tab_height = available_height - collapsed_tabs_height - BORDER_MARGIN;
