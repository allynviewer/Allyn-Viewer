...
std::string LLUrlEntryParcel::getLabel(const std::string &url, const LLUrlLabelCallback &cb)
{
	...
	std::string parcel_id_string = unescapeUrl(path_array[2]);
...
std::string LLUrlEntryRegion::getLabel(const std::string &url, const LLUrlLabelCallback &cb)
{
	...
	std::string label =
		xgrid ? unescapeUrl(path_array[0].asStringRef()) + ": " + unescapeUrl(path_array[i].asStringRef()) :
		unescapeUrl(path_array[i].asStringRef());
	++i;
	if (path_parts > i+1)
	{
...
std::string LLUrlEntryJira::getUrl(const std::string &url) const
{
	return (boost::format(
		...
		(url.find("SV") != std::string::npos) ?
			"https://AllynViewer.atlassian.net/browse/%1%" :
