...
const S32	SCROLL_INCREMENT_ADD = 0;
const S32   SCROLL_INCREMENT_DEL = 4;
...
LLPointer<LLUIImage> LLLineEditor::sImage;
...
void LLLineEditor::draw()
{
	...
	if (mImage.notNull())
	{
		if (hasFocus())
		{
			mImage->drawBorder(0, 0, getRect().getWidth(), getRect().getHeight(),
				gFocusMgr.getFocusColor() % alpha, gFocusMgr.getFocusFlashWidth());
		}
		if (!mReadOnly)
		{
			bg_color = (gFocusMgr.getKeyboardFocus() == this) ? mFocusBgColor : mWriteableBgColor;
		}
		mImage->draw(getLocalRect(), bg_color % alpha);
		if (mBorder)
		{
	...
	if (mVAlign == LLFontGL::VCENTER)
	{
		text_bottom = (F32)background.mBottom +
			llmax(0.f, ((F32)background.getHeight() - mGLFont->getLineHeight()) * 0.5f);
	}
	S32 cursor_bottom = background.mBottom + 1;
	S32 cursor_top = background.mTop - 1;
	if (mVAlign == LLFontGL::VCENTER)
	{
		cursor_bottom = llfloor(text_bottom);
		cursor_top = llceil(text_bottom + mGLFont->getLineHeight());
	}
	LLColor4 text_color;
	if (!mReadOnly)
	{
	...
	if( gFocusMgr.getKeyboardFocus() == this)
	{
		if (gShowTextEditCursor && !mReadOnly)
		{
			...
			if( (elapsed < CURSOR_FLASH_DELAY ) || (S32(elapsed * 2) & 1) )
			{
				...
				S32 ime_y = (mVAlign == LLFontGL::VCENTER)
					? screen_pos.mBottom + ll_round(text_bottom)
					: screen_pos.mTop - UI_LINEEDITOR_V_PAD;
				LLCoordGL ime_pos( screen_pos.mLeft + pixels_after_scroll, ime_y );
...
LLView* LLLineEditor::fromXML(LLXMLNodePtr node, LLView *parent, LLUICtrlFactory *factory)
{
	...
	{
		line_editor->setVAlign(LLView::selectFontVAlign(node));
	}
