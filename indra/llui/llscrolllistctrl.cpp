...
void LLScrollListCtrl::updateLayout()
{
	...
	adjustScrollbar(mFilter.empty() ? getItemCount() : mScrollbar->getDocSize());
	dirtyColumns();
}
...
{
	if (!mPopupMenu) return FALSE;
	LLScrollListItem *item = hitItem(x, y);
	if (item)
	{
		if (!item->getSelected()) selectItem(item);
	}
	else if (LLScrollListColumn* col = getColumn(getColumnIndexFromOffset(x)))
	{
		if (col->mHeader && col->mHeader->getRect().pointInRect(x,y))
			return FALSE;
	}
...
void LLScrollListCtrl::setFilter(const std::string& filter)
{
	...
	mFilter = filter;
	S32 unfiltered_count = no_filter ? mItemList.size()
		: !unique ? mScrollbar->getDocSize()
		: 0;
	for (auto& item : mItemList)
	{
		...
		{
			if (filterItem(item)) --unfiltered_count;
		}
		else if (unique	||
				(contracted && item->getFiltered()))
		{
			if (!filterItem(item)) ++unfiltered_count;
		}
	}
	if (mLastSelected && mLastSelected->getFiltered())
