...
void LLScrollbar::draw()
{
	...
	else
	{
		rounded_rect_imagep->drawSolid(mOrientation == HORIZONTAL ? SCROLLBAR_SIZE : 0,
			...
			mTrackColor);
		LLRect thumb = mThumbRect;
		if (mOrientation == VERTICAL)
		{
			S32 inset = llmax(3, (thumb.getWidth() - 6) / 2);
			thumb.mLeft += inset;
			thumb.mRight -= inset;
		}
		else
		{
			S32 inset = llmax(3, (thumb.getHeight() - 6) / 2);
			thumb.mBottom += inset;
			thumb.mTop -= inset;
		}
		if (gFocusMgr.getKeyboardFocus() == this)
		{
			LLRect outline_rect = thumb;
			outline_rect.stretch(2);
			rounded_rect_imagep->draw(outline_rect, gFocusMgr.getFocusColor());
		}
		rounded_rect_imagep->draw(thumb, mThumbColor);
		if (mCurGlowStrength > 0.01f)
		{
			gGL.setSceneBlendType(LLRender::BT_ADD_WITH_ALPHA);
			rounded_rect_imagep->drawSolid(thumb, LLColor4(1.f, 1.f, 1.f, mCurGlowStrength));
