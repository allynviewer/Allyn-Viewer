...
LLControlGroup* LLUI::sAccountGroup = NULL;
...
std::string LLUI::locateSkin(const std::string& filename)
{
	...
	found_file = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, filename);
	if (gDirUtilp->fileExists(found_file))
	{
		return found_file;
	}
...
namespace LLInitParam
{
	...
	void ParamValue<LLUIColor>::updateValueFromBlock()
	{
		...
		{
			updateValue(LLUI::sColorsGroup->controlExists(control) ? LLUI::sColorsGroup->getColor(control) : LLUI::sConfigGroup->getColor(control));
		}
		else
		{
