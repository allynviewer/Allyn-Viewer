...
LLButton::Params::Params()
	...
	image_color("image_color", LLUI::sColorsGroup->getColor("ButtonImageColor")),
...
void LLButton::draw()
{
	...
	bool flash = FALSE;
	F32 unread_pulse_t = 0.f;
	if( mFlashing )
	{
		F32 elapsed = mFlashingTimer.getElapsedTimeF32();
		if (mFlashLabelSubtle)
		{
			unread_pulse_t = 0.5f + 0.5f * sinf(elapsed * F_TWO_PI * 0.7f);
		}
		else
		{
	...
	if (mFlashing && !mFlashLabelSubtle)
	{
		...
		else
		{
			...
			use_glow_effect = TRUE;
			glow_type = LLRender::BT_ALPHA;
			if (mNeedsHighlight)
				glow_color = (glow_color*0.5f + flash_color*0.5f) % 2.0f;
			else
	...
	std::string unread_label;
	S32 unread_width = 0;
	if (mUnreadCount > 0 && !getToggleState())
	{
		if (mUnreadCount > 99)
		{
			unread_label = "99+";
		...
		{
			unread_label = llformat("%d", mUnreadCount);
		}
		unread_width = mGLFont->getWidth(unread_label) + 4;
	}
	if (hasFocus())
	{
	...
	if (mFlashing && mFlashLabelSubtle && !getToggleState() && !hasFocus())
	{
		static const LLColor4 unread_orange(1.f, 92.f / 255.f, 0.f, 1.f);
		const S32 w = getRect().getWidth();
		const S32 h = getRect().getHeight();
		const S32 bar_w = 3;
		const S32 underline_h = 2;

		LLColor4 wash = unread_orange;
		wash.mV[VALPHA] = (0.06f + 0.08f * unread_pulse_t) * alpha;
		gl_rect_2d(0, h, w, 0, wash, TRUE);

		LLColor4 bar = unread_orange;
		bar.mV[VALPHA] = (0.55f + 0.40f * unread_pulse_t) * alpha;
		gl_rect_2d(0, h - 1, bar_w, 1, bar, TRUE);

		LLColor4 underline = unread_orange;
		underline.mV[VALPHA] = (0.35f + 0.35f * unread_pulse_t) * alpha;
		gl_rect_2d(bar_w, underline_h, w, 0, underline, TRUE);
	}
	...
	if (unread_width > 0)
	{
		text_right -= unread_width;
		text_width = llmax(0, text_width - unread_width);
	}
	...
	if (!unread_label.empty())
	{
		S32 y_offset = 2 + (getRect().getHeight() - 20)/2;
		static const LLColor4 unread_orange(1.f, 92.f / 255.f, 0.f, 1.f);
		LLColor4 unread_color = unread_orange;
		unread_color.mV[VALPHA] = 0.85f * alpha;
		mGLFont->renderUTF8(unread_label, 0,
			(F32)(getRect().getWidth() - mRightHPad),
			(F32)(mBottomVPad + y_offset),
			unread_color,
...
void LLButton::setFlashing( BOOL b, bool label_subtle )
{
	mFlashLabelSubtle = b && label_subtle;
	if ((bool)b != mFlashing)
	{
		mFlashing = b;
...
void LLButton::setUnreadCount( S32 count )
{
	mUnreadCount = llmax(0, count);
}
...
void LLButton::setUsePrimaryChrome(BOOL use_primary)
{
	if (use_primary)
	{
		setImageUnselected(LLUI::getUIImage("button_enabled_selected_32x128.tga"));
	}
	else
	{
		setImageUnselected(LLUI::getUIImage("button_enabled_32x128.tga"));
	}
}
