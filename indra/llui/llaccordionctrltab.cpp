...
public:
	...
	struct Params : public LLAccordionCtrlTab::Params
	{
		Params();
...
void LLAccordionCtrlTab::LLAccordionCtrlTabHeader::setTitleFontStyle(std::string style)
{
	if (mHeaderTextbox)
	{
		mHeaderTextbox->setFontStyle( LLFontGL::getStyleFromString(style));
	}
}
...
void LLAccordionCtrlTab::LLAccordionCtrlTabHeader::reshape(S32 width, S32 height, BOOL called_from_parent )
{
...
void LLAccordionCtrlTab::reshape(S32 width, S32 height, BOOL called_from_parent )
{
	LLRect headerRect;
...
LLView* LLAccordionCtrlTab::fromXML(LLXMLNodePtr node, LLView* parent, LLUICtrlFactory* factory)
{
	...
	{
		p.header_bg_color(LLUI::sColorsGroup->getColor("ButtonUnselectedBgColor"));
	}
	...
	{
		p.header_text_color(LLUI::sColorsGroup->getColor("ButtonLabelColor"));
	}
