...
S32 LLLayoutStack::getPanelIndex(const LLView* panel) const
{
	for (S32 i = 0; i < (S32)mPanels.size(); ++i)
	{
		if (mPanels[i] == panel)
		{
			return i;
...
LLLayoutPanel* LLLayoutStack::getPanelByIndex(S32 index) const
{
	if (index < 0 || index >= (S32)mPanels.size())
	{
		return NULL;
	}
	return mPanels[index];
}
void LLLayoutStack::movePanelRange(S32 from_index, S32 count, S32 insert_index)
{
	if (from_index < 0 || count <= 0 || from_index + count > (S32)mPanels.size())
	{
		return;
	}
	if (insert_index < 0)
	...
	if (insert_index > (S32)mPanels.size())
	{
		insert_index = (S32)mPanels.size();
	}
	if (insert_index >= from_index && insert_index <= from_index + count)
	{
		return;
	}
	e_panel_list_t moving(mPanels.begin() + from_index, mPanels.begin() + from_index + count);
	mPanels.erase(mPanels.begin() + from_index, mPanels.begin() + from_index + count);
	if (insert_index > from_index)
	{
		insert_index -= count;
	}
	mPanels.insert(mPanels.begin() + insert_index, moving.begin(), moving.end());
	mNeedsLayout = true;
}
...
void LLLayoutStack::updateResizeBarLimits()
{
	...
	for(LLLayoutPanel* visible_panelp : boost::adaptors::reverse(mPanels))
	{
		...
		if (previous_visible_panelp
			&& (visible_panelp->mUserResize || previous_visible_panelp->mUserResize)
			&& (visible_panelp->mAutoResize || visible_panelp->mUserResize)
			&& (previous_visible_panelp->mAutoResize || previous_visible_panelp->mUserResize))
		{
