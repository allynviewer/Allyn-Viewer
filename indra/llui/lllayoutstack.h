...
public:
	...
	void draw();
	void removeChild(LLView*);
	BOOL postBuild();
	bool addChild(LLView* child, S32 tab_groupdatefractuiona = 0);
	...
	S32 getPanelIndex(const LLView* panel) const;
	LLLayoutPanel* getPanelByIndex(S32 index) const;
	void movePanelRange(S32 from_index, S32 count, S32 insert_index);
	void updateLayout();
