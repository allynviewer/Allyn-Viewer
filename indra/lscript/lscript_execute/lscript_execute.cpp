...
const char* LSCRIPTRunTimeFaultStrings[LSRF_EOF] =
{
	"Invalid",
	"Math Error",
...
void integer_integer_operation(U8 *buffer, LSCRIPTOpCodesEnum opcode)
{
	...
	case LOPC_DIV:
		if (rside){
			if( ( rside == -1 ) || ( rside == (S32) 0xffffffff ) )
			{
				result = -1 * lside;
			}
			else
			...
			case LST_STRING:
				{
					...
					snprintf(caststr, sizeof(caststr), "%d", source);
			...
			case LST_STRING:
				{
					S32 address;
					F32 source = lscript_pop_float(buffer);
					snprintf(caststr, sizeof(caststr), "%f", source);
					address = lsa_heap_add_data(buffer, new LLScriptLibData(caststr), get_max_heap_size(buffer), TRUE);
					lscript_push(buffer, address);
				}
				break;
			...
			case LST_STRING:
				{
					...
					snprintf(caststr, sizeof(caststr), "<%5.5f, %5.5f, %5.5f>", source.mV[VX], source.mV[VY], source.mV[VZ]);
					address = lsa_heap_add_data(buffer, new LLScriptLibData(caststr), get_max_heap_size(buffer), TRUE);
					lscript_push(buffer, address);
				}
				break;
			...
			case LST_STRING:
				{
					...
					snprintf(caststr, sizeof(caststr), "<%5.5f, %5.5f, %5.5f, %5.5f>", source.mQ[VX], source.mQ[VY], source.mQ[VZ], source.mQ[VS]);
					address = lsa_heap_add_data(buffer, new LLScriptLibData(caststr), get_max_heap_size(buffer), TRUE);
					lscript_push(buffer, address);
				}
				break;
...
BOOL run_calllib_common(U8 *buffer, S32 &offset, const LLUUID &id, U16 arg)
{
	...
	if (function.mArgs)
	{
		number = (S32)strlen(function.mArgs);
