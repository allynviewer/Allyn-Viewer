...
namespace
{
bool isCefHostProcess(DWORD pid)
{
	...
	if (QueryFullProcessImageNameW(process, 0, path, &size))
	{
		...
		ours = image.find(L"SLplugin.exe") != std::wstring::npos
			|| image.find(L"dullahan_host.exe") != std::wstring::npos;
	}
	CloseHandle(process);
	return ours;
}
BOOL CALLBACK hideStrayCefChromeWindowProc(HWND hwnd, LPARAM)
{
...
void hideStrayCefChromeWindows()
{
	EnumWindows(hideStrayCefChromeWindowProc, 0);
}
}
#endif
...
private:
	...
	void onLoadEndCallback(int httpStatusCode, const std::string url);
...
void MediaPluginCEF::onCustomSchemeURLCallback(std::string url, bool , bool )
{
...
{
	if (in.empty() || in == "en" || in == "en-us" || in == "en-gb" || in == "en-GB")
		return "en-US";
	if (in == "pt" || in == "pt-PT" || in == "pt-pt")
		return "pt-BR";
	if (in == "de" || in == "es" || in == "fr" || in == "it" ||
	    in == "tr" || in == "ru" || in == "ja")
		return in;
	return "en-US";
}
	...
	if (message_in.parse(message_string) >= 0)
	{
		...
		if (message_class == LLPLUGIN_MESSAGE_CLASS_BASE)
		{
			...
			else if (message_name == "idle")
			{
				...
				{
					hideStrayCefChromeWindows();
					--mHideChromeWindowsTicks;
				}
#endif
				checkEditState();
			}
		...
		else if (message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
		{
			if (message_name == "init")
			{
				...
				settings.root_cache_path = mCachePath.empty() ? mUserDataPath : mCachePath;
				settings.cookies_enabled = mCookiesEnabled;
				settings.disable_gpu = mDisableGPU;
				settings.begin_frame_scheduling = true;
				...
				{
					settings.proxy_host_port = mProxyHost + ":" + std::to_string(mProxyPort);
				}
			...
			else if (message_name == "set_user_data_path")
			{
				...
				const std::string path_separator = "/";
#endif
				// Firestorm/SL GHI #4498: unique cef_cache/<pid> per CEF process.
				// Viewer deletes these at next startup so disk does not grow forever.
				mUserDataPath = user_data_path_cache + "cef_data";
				mCachePath = user_data_path_cache + "cef_cache" + path_separator
#if LL_WINDOWS
