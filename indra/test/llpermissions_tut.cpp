...
namespace tut
{
	...
	void permission_object_t::test<3>()
	{
		LLPermissions permissions;
		U32 base = PERM_ALL;
		U32 owner = PERM_ITEM_UNRESTRICTED;
		U32 group = PERM_TRANSFER | PERM_MOVE | PERM_COPY|PERM_MODIFY;
		U32 everyone = PERM_TRANSFER | PERM_MOVE | PERM_MODIFY;
		U32 next = PERM_NONE;
		U32 fixedbase = base;
		U32 fixedowner = PERM_ITEM_UNRESTRICTED;
		U32 fixedgroup = PERM_ITEM_UNRESTRICTED;
		U32 fixedeveryone = PERM_TRANSFER;
		U32 fixednext = PERM_NONE;
		permissions.initMasks(base, owner, everyone, group, next);
