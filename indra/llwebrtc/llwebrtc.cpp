...
LLWebRTCAudioTransport::LLWebRTCAudioTransport() : mMicrophoneEnergy(0.0)
{
    ...
    mLoopbackBuffer.assign(LOOPBACK_CAPACITY, 0);
}
void LLWebRTCAudioTransport::SetEngineTransport(webrtc::AudioTransport* t)
{
    engine_.store(t, std::memory_order_release);
}
void LLWebRTCAudioTransport::SetTuningMonitor(bool enable)
{
    mTuningMonitor.store(enable, std::memory_order_release);
    std::lock_guard<std::mutex> lock(mLoopbackMutex);
    mLoopbackRead  = 0;
    mLoopbackWrite = 0;
    mLoopbackCount = 0;
    std::fill(mLoopbackBuffer.begin(), mLoopbackBuffer.end(), 0);
}
void LLWebRTCAudioTransport::pushLoopbackSamples(const int16_t* samples, size_t sample_count, float gain)
{
    ...
    for (size_t i = 0; i < sample_count; ++i)
    {
        ...
        if (scaled < -32768.f)
            scaled = -32768.f;
        mLoopbackBuffer[mLoopbackWrite] = static_cast<int16_t>(scaled);
        mLoopbackWrite = (mLoopbackWrite + 1) % LOOPBACK_CAPACITY;
        if (mLoopbackCount < LOOPBACK_CAPACITY)
        {
            ++mLoopbackCount;
        }
        else
        {
            mLoopbackRead = (mLoopbackRead + 1) % LOOPBACK_CAPACITY;
        }
    }
}
size_t LLWebRTCAudioTransport::pullLoopbackSamples(int16_t* out, size_t sample_count)
{
    ...
    std::lock_guard<std::mutex> lock(mLoopbackMutex);
    size_t produced = 0;
    while (produced < sample_count && mLoopbackCount > 0)
    {
        out[produced++] = mLoopbackBuffer[mLoopbackRead];
        mLoopbackRead   = (mLoopbackRead + 1) % LOOPBACK_CAPACITY;
        --mLoopbackCount;
    }
    if (produced < sample_count)
    {
        memset(out + produced, 0, (sample_count - produced) * sizeof(int16_t));
    }
    return sample_count;
}
                                                        ...
                                                        uint32_t&   new_mic_level)
{
    ...
    int32_t ret = 0;
    if (engine && !mTuningMonitor.load(std::memory_order_acquire))
    {
    ...
    if (mTuningMonitor.load(std::memory_order_acquire) && number_of_channels > 0)
    {
        const float monitor_gain = std::max(0.6f, gain) * 0.85f;
        std::vector<int16_t> mono(number_of_frames);
        for (size_t frame = 0; frame < number_of_frames; ++frame)
        {
            int32_t sum = 0;
            for (size_t ch = 0; ch < number_of_channels; ++ch)
            {
                sum += samples[frame * number_of_channels + ch];
            }
            mono[frame] = static_cast<int16_t>(sum / static_cast<int32_t>(number_of_channels));
        }
        pushLoopbackSamples(mono.data(), mono.size(), monitor_gain);
    }
    return ret;
}
int32_t LLWebRTCAudioTransport::NeedMorePlayData(size_t   number_of_frames,
                                                 ...
                                                 int64_t* ntp_time_ms)
{
    if (mTuningMonitor.load(std::memory_order_acquire))
    {
        std::vector<int16_t> mono(number_of_frames);
        pullLoopbackSamples(mono.data(), number_of_frames);
        int16_t* out = static_cast<int16_t*>(audio_data);
        for (size_t frame = 0; frame < number_of_frames; ++frame)
        {
            for (size_t ch = 0; ch < number_of_channels; ++ch)
            {
                out[frame * number_of_channels + ch] = mono[frame];
            }
        }
...
void LLWebRTCAudioTransport::PullRenderData(int      bits_per_sample,
                                            ...
                                            int64_t* ntp_time_ms)
{
    if (mTuningMonitor.load(std::memory_order_acquire) && audio_data && number_of_channels > 0)
    {
        std::vector<int16_t> mono(number_of_frames);
        pullLoopbackSamples(mono.data(), number_of_frames);
        ...
        {
            memset(audio_data, 0, number_of_frames * number_of_channels * (bits_per_sample / 8));
        }
        if (elapsed_time_ms)
            *elapsed_time_ms = 0;
        if (ntp_time_ms)
    ...
    {
        memset(audio_data, 0, number_of_frames * number_of_channels * std::max(1, bits_per_sample / 8));
    }
}
...
void LLWebRTCPeerConnectionImpl::OnSuccess(webrtc::SessionDescriptionInterface *desc)
{
    ...
    while (std::getline(sdp_stream, sdp_line))
    {
        ...
        int payload_id = 0;
#if defined(_MSC_VER)
        if (sscanf_s(sdp_line.c_str(), "a=rtpmap:%i opus/%i/2", &payload_id, &bandwidth) == 2)
#else
