				  ...
				  LLVector3 &intersection, LLVector3 &intersection_normal)
{
	...
	shortest_distance = shortest_direction.normVec();
	...
	if (shortest_distance <= cyl_radius)
	{
		F32 		dist_to_closest_point;
		F32 		half_chord_length;
		F32 		in, out;
		...
		out = dist_to_closest_point + half_chord_length;
		if (out < 0.0f)
		{
			return FALSE;
		}
		in = dist_to_closest_point - half_chord_length;
		if (in < 0.0f)
		{
