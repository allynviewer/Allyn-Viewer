...
void LLProfile::genNGon(const LLProfileParams& params, S32 sides, F32 offset, F32 bevel, F32 ang_scale, S32 split)
{
	...
	S32 total_sides = ll_pos_round(sides / ang_scale);
	if (total_sides < 8)
	{
...
bool LLVolumeParams::setBeginAndEndS(const F32 b, const F32 e)
{
	...
	F32 end = e;
	if (end >= .0149f && end < MIN_CUT_DELTA) end = MIN_CUT_DELTA;
...
BOOL LLVolumeFace::createSide(LLVolume* volume, BOOL partial_build)
{
	...
	BOOL sculpt_invert = sculpt_type & LL_SCULPT_FLAG_INVERT;
	BOOL sculpt_mirror = sculpt_type & LL_SCULPT_FLAG_MIRROR;
	BOOL sculpt_reverse_horizontal = (sculpt_invert ? !sculpt_mirror : sculpt_mirror);
	...
	if (!partial_build)
	{
		for (t = 0; t < (mNumT-1); t++)
		{
			for (s = 0; s < (mNumS-1); s++)
			{
				mIndices[cur_index++] = s   + mNumS*t;
				mIndices[cur_index++] = s+1 + mNumS*(t+1);
				mIndices[cur_index++] = s   + mNumS*(t+1);
				mIndices[cur_index++] = s   + mNumS*t;
				mIndices[cur_index++] = s+1 + mNumS*t;
				mIndices[cur_index++] = s+1 + mNumS*(t+1);
				mEdge[cur_edge++] = (mNumS-1)*2*t+s*2+1;
				if (t < mNumT-2) {
					mEdge[cur_edge++] = (mNumS-1)*2*(t+1)+s*2+1;
				}
				else if (mNumT <= 3 || volume->getPath().isOpen() == TRUE) {
					mEdge[cur_edge++] = -1;
				}
				else {
				...
				else if (flat_face ||  volume->getProfile().isOpen() == TRUE) {
					mEdge[cur_edge++] = -1;
				}
				else {
