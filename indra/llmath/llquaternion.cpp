...
{
	const F32 cos_angle_lim = cosf( radians/2 );
	const F32 sin_angle_lim = sinf( radians/2 );
	if (mQ[VW] < 0.f)
	{
		mQ[VX] *= -1.f;
		mQ[VY] *= -1.f;
	...
	{
		mQ[VW] = cos_angle_lim;
		F32 axis_len = sqrtf( mQ[VX]*mQ[VX] + mQ[VY]*mQ[VY] + mQ[VZ]*mQ[VZ] );
...
{
	F32 sx = 2 * (mQ[VX] * mQ[VW] - mQ[VY] * mQ[VZ]);
	F32 sy = 2 * (mQ[VY] * mQ[VW] + mQ[VX] * mQ[VZ]);
	F32 ys = mQ[VW] * mQ[VW] - mQ[VY] * mQ[VY];
	F32 xz = mQ[VX] * mQ[VX] - mQ[VZ] * mQ[VZ];
	F32 cx = ys - xz;
	F32 cy = sqrtf(sx * sx + cx * cx);
