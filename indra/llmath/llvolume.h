...
public:
	...
	const F32& getTwist() const			{ return mTwistEnd; }
	...
	void setTwist(const F32 twist)				{ setTwistEnd(twist); }
...
public:
	...
	bool setRatio(const F32 x)					{ return setRatio(x,x); }
	bool setShear(const F32 x)					{ return setShear(x,x); }
	...
	bool setTwist(const F32 twist)				{ return setTwistEnd(twist); }
...
public:
	...
	S32	 getTotalOut() const							{ return mTotalOut; }
...
public:
	...
	const LLVolumeFace &getVolumeFace(const S32 f) const {return mVolumeFaces[f];}
	LLVolumeFace &getVolumeFace(const S32 f) {return mVolumeFaces[f];}
