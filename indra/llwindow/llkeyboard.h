...
public:
	...
	static BOOL		maskFromString(const std::string& str, MASK *mask);
	static BOOL		keyFromString(const std::string& str, KEY *key);
...
protected:
	std::map<U16, KEY>	mTranslateKeyMap;
	std::map<KEY, U16>	mInvTranslateKeyMap;
	...
	BOOL			mControllerKeys[KEY_COUNT];
